...
//! Test-only ADR-0100/ADR-0207 legacy changelog compatibility evidence.
//!
...
// Reuse canonical first-party codecs and key types; do not duplicate semantics.
#[allow(dead_code)] // The legacy compatibility fixture uses only a codec subset.
#[path = "../../crates/riffdb-storage-redb/src/codec.rs"]
mod codec;
#[allow(dead_code)] // The legacy compatibility fixture uses only an error subset.
#[path = "../../crates/riffdb-storage-redb/src/error.rs"]
mod error;
#[path = "../../crates/riffdb-storage-redb/src/keys.rs"]
mod keys;

const META_DATABASE_ID: &str =
    match riffdb_storage_api::AuthoritativeNamespaceV1::DatabaseIdentity.metadata_key() {
        Some(key) => key,
        None => panic!("database identity is metadata"),
    };
const META_HISTORY_INCARNATION: &str =
    match riffdb_storage_api::AuthoritativeNamespaceV1::HistoryIncarnation.metadata_key() {
        Some(key) => key,
        None => panic!("history incarnation is metadata"),
    };
...
pub(super) const DEFAULT_CHANGELOG_BUFFER_ADVANCEMENTS: usize = 64;
...
pub(super) struct RedbChangelogEmitter {
...
impl RedbChangelogEmitter {
    ...
    pub(super) fn wait_for_emitted(&self, frames: u64) -> ChangelogEmissionStateV1 {
        let mut progress = self.locked_progress();
    ...
    pub(super) fn wait_for_resync(&self) -> ChangelogResyncReasonV1 {
        let mut progress = self.locked_progress();
        loop {
...
pub(super) struct RedbChangelogEmitterHandle {
...
impl RedbChangelogEmitterHandle {
    ...
    #[must_use]
    pub(super) fn emitter(&self) -> &Arc<RedbChangelogEmitter> {
        &self.emitter
    }

    /// Returns the publication port to inject at store bind.
    #[must_use]
    pub(super) fn port(&self) -> Arc<dyn ChangelogPublicationPort> {
...
pub(super) fn start_changelog_emitter(
...
pub(super) fn start_changelog_emitter_v2(
...
fn read_identity(
    ...
    let database_id = *self::codec::decode_database_identity_v1(&database_row)
        .map_err(|_| ChangelogResyncReasonV1::DerivationFailed)?
        .value();
    let incarnation_row = snapshot
        ...
        .map_err(|_| ChangelogResyncReasonV1::DerivationFailed)?
        .ok_or(ChangelogResyncReasonV1::DerivationFailed)?;
    let history_incarnation = *self::codec::decode_history_incarnation_v1(&incarnation_row)
        .map_err(|_| ChangelogResyncReasonV1::DerivationFailed)?
        .value();
...
pub(crate) fn derive_frame_v2(
    ...
    for transition in &attribution.entity_transitions {
        ...
        attribution
            .entities
            .push(Box::from(self::keys::encode_entity_key(
...
fn attribute_commit_row(
    ...
    let references = self::codec::decode_commit_entity_references(encoded)
        .map_err(|_| ChangelogResyncReasonV1::DerivationFailed)?;
...
fn attribute_audit_row(
    ...
    let record = self::codec::decode_administration_audit_record_v1(encoded)
        .map_err(|_| ChangelogResyncReasonV1::DerivationFailed)?;
