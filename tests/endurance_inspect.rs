...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match checkpoint {
        Some(sequence) => println!(
            "{{\"schema\":\"riffdb.alpha-endurance-storage-inspection/v1\",\"checkpoint_commit_sequence\":{sequence}}}"
        ),
        None => println!(
            "{{\"schema\":\"riffdb.alpha-endurance-storage-inspection/v1\",\"checkpoint_commit_sequence\":null}}"
        ),
    }
    Ok(())
}
