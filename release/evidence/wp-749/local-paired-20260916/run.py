...
root=Path('/home/kevin/tmp/wp749-local-paired-20260916')
root.mkdir(exist_ok=False)
(root/'tmp').mkdir()
identities=json.loads((inputs/'identities.json').read_text())
runner=inputs/'riffdb-app-baseline'
current=Path('/home/kevin/tmp/wp749-measure-current')
control=Path('/home/kevin/tmp/wp749-prefix-cost-control')
...
args=['--full','--load','write_only','--load-clients','32','--load-duration-secs','90','--load-warmup-secs','15','--reps','3','--require-stable','--skip-postgres','--database-root','/home/kevin/tmp/wp749-measure-db']
...
for name,cwd,variant,mode in cells:
 ...
 env={'PATH':'/home/kevin/.cargo/bin:/usr/bin:/bin','HOME':'/home/kevin','CARGO_HOME':'/home/kevin/.cargo','LC_ALL':'C','TZ':'UTC','RIFFDB_TMP_ROOT':str(root/'tmp'),'RIFFDB_REDB_COMMIT_PROFILE':'standard','RIFFDB_APP_BASELINE_RIFFDBD_BIN':str(daemon),'RIFFDB_APP_BASELINE_RUNNER_BIN':str(runner)}
