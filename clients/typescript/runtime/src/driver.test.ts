...
test("an empty-database query preserves application frontier zero", async () => {
  ...
  try {
    ...
    const result = await transport.invoke(
      { name: "better_auth_find_user_by_email", inputSchemaHash: HASH },
      { email: { type: "string", value: "first@example.test" } },
    );
...
test("compact query results preserve compiler order and reject malformed rows", async () => {
  ...
  for (const response of [
    ...
    try {
      await assert.rejects(
        client.invoke({ name: "ticketdesk_board_page450", inputSchemaHash: HASH }, {}, { acceptCompactResult: true }),
        /invalid compact result/u,
      );
    } finally {
...
test("query frontiers reject values outside the closed non-negative u64 domain", async (context) => {
  for (const [name, applicationHead] of [
    ...
    await context.test(name, async () => {
      ...
      try {
        await assert.rejects(
          transport.invoke({ name: "ticketdesk_invalid_frontier", inputSchemaHash: HASH }, {}),
          /invalid (?:RiffDB driver message|message)/u,
        );
      } finally {
        await transport.shutdown();
        await fixture.close();
...
test("positive-only driver identities continue to reject zero", async (context) => {
  await context.test("batch commit sequence", async () => {
    ...
    try {
      await assert.rejects(
        transport.batch({ name: "ticketdesk_create", inputSchemaHash: HASH }, [{}], 1, 0),
        /invalid RiffDB driver message/u,
      );
    } finally {
      await transport.shutdown();
      await fixture.close();
  ...
  await context.test("error contract version", async () => {
    ...
    try {
      await assert.rejects(
        transport.invoke({ name: "ticketdesk_find", inputSchemaHash: HASH }, {}),
        /invalid RiffDB driver message/u,
      );
    } finally {
      await transport.shutdown();
