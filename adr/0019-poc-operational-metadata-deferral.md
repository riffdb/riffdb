...
both graceful and ungraceful prior termination. No marker, remembered integrity
result, elapsed-time rule, or process history may skip or weaken that validation.
The validation reports or fails closed on findings and performs no online repair.
...
allowed paths, requirements, acceptance commands, and gate membership remain
unchanged.
...
- Startup pays the bounded cost of authoritative integrity validation every
  time. A graceful previous shutdown does not provide a fast-path exemption.
...
- WP-070 startup tests prove that integrity validation runs after both graceful
  close and crash, never performs authoritative repair, validates every retained
  metadata invariant, and does not write a shutdown or integrity-history marker.
- WP-185 composition tests preserve graceful shutdown without adding a metadata
  write or an integrity-skip path.
...
- **Defines or unblocks:** metadata interface in `WP-060`; durable schema review
  in `WP-065`; startup integrity and shutdown persistence behavior in `WP-070`;
  composition evidence in `WP-185`; recovery evidence in `WP-190`
...
the already declared lifecycle and recovery interfaces.
...
3. every startup performs read-only integrity validation before readiness;
