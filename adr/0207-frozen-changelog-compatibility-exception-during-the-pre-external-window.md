...
obligations:
  ...
  - id: OBL-0207-2
    ...
    says: V1 and V2 remain read-only compatibility decoders and fixtures while every production
      negotiation, emission, apply, bootstrap tail, and archive path selects only exact V3.
review_triggers:
...
The retained formats prove that V1 remains put-only, V2 preserves delete-aware transition and
rotation semantics, neither old identity is reinterpreted as V3, and V3 refuses downgrade rather
than claiming incomplete replication. They do not serve an external database, authorize a legacy
production path, or justify closing the pre-external window. A closed topology exception preserves
that negative evidence without reopening the general multi-reader carrying cost ADR-0181 removes.
...
2. The domain's lifecycle is exact: V3 is its sole `active` identity; V1 and V2 are exactly
   `read_only`; `retirement_candidate` and `retired` are empty. V1/V2 source decoders and frozen
   fixtures remain only for the ADR-0100 and ADR-0186 compatibility evidence. Their bytes, magic,
   tags, chains, rotation receipt, checksums, and interpretations remain immutable.

3. Topology readability grants no production selection. Every production replication handshake,
   emission, follower apply, bootstrap tail, and WP-749 archive writes, accepts, and persists only
   exact V3 under ADR-0186. V1/V2 never provide fallback, translation, bootstrap, restore, archive,
   downgrade, or a partial-authority mode. An unsupported or mismatched V3 format, catalog,
   lineage, epoch, bound, or position refuses before transfer or mutation.
...
8. After exact acceptance, one separate governance commit updates WP-757: add ADR-0207 to
   ...
   topology deliverable to the exact domain, sets, lifecycle, and V3-only production rule; and add
   exception drift to its human review triggers. Dependencies, allowed paths, requirements, other
   deliverables, and acceptance commands remain unchanged.
...
9. WP-757 implements `topology_allows_only_exact_changelog_compatibility_exception` in
   ...
   another exceptional domain; any missing, extra, duplicate, or reordered identity; V1/V2 in
   writable, current, or active; V3 absent from any required set or lifecycle; a nonempty candidate
   or retired set; a different writer policy; and any configurable exception field. The existing
   paired nonempty-`external_databases` case continues to prove ordinary ADR-0124 behavior after the
   window closes. WP-772 separately proves that no production path selects V1 or V2.
...
1. **Retire V1/V2 and make V3 the only reader:** rejected because it reopens ADR-0100, ADR-0178,
   and ADR-0186, removes the exact negative compatibility evidence, and requires broad runtime and
   recovery-test surgery merely to satisfy a repository rule.
...
- V1/V2 compatibility evidence remains explicit and reviewable while V3 is the sole production
  replication and archive identity.
...
- `changelog_v3_is_only_production_replication_identity` proves V1/V2 are compatibility-only and
  unreachable from production negotiation, emission, apply, bootstrap tail, and archive paths.
