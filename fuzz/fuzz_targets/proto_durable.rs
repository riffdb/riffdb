...
use riffdb_proto::{durable::readable_record_registry, envelope::MAX_STORED_ENVELOPE_BYTES};
use riffdb_storage_api::proto_codec::{
    ...
    decode_index_entry_v2, decode_index_epoch_v1, decode_index_migration_row,
...
fuzz_target!(|input: &[u8]| {
    ...
    let _ = readable_record_registry().decode(input);
    ...
    if let Ok(row) = decode_index_entry_v1(input) {
        let _ = decode_index_migration_row(row.value().key(), input);
    }
    if let Ok(row) = decode_index_entry_v2(input) {
        let _ = decode_index_migration_row(row.value().key(), input);
    }
