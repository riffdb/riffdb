...
pub const CONTRACT_BUNDLE_HASH: [u8; 32] = [0x7c, 0xae, 0x11, 0x22, 0xf8, 0xda, 0xad, 0x04, 0x69, 0xb7, 0x4e, 0x1e, 0x40, 0x47, 0x3f, 0x7c, 0xfc, 0xd9, 0xea, 0x84, 0x74, 0xc3, 0x9e, 0x1d, 0xdc, 0x51, 0x02, 0x18, 0xb2, 0xac, 0x09, 0x99];
...
pub const BOARD_PAGE200_QUERY_PLAN_HASH: [u8; 32] = [0xcb, 0x06, 0xf3, 0x30, 0x07, 0x91, 0x61, 0x4d, 0xaa, 0x6b, 0xb7, 0x2e, 0x5a, 0x8d, 0x85, 0x05, 0x27, 0xbd, 0xe7, 0x9c, 0x63, 0xa0, 0x8c, 0x2d, 0x03, 0xaa, 0xcc, 0xb6, 0xe7, 0x3b, 0xfc, 0x47];
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl GeneratedQuery for BoardPage200Query {
    ...
    fn decode_compact_result(outcome: String, response: app_v1::CompactResultField) -> Result<Self::Output, ApplicationClientError> {
        if outcome != "Found"
            ...
            || response.fields != vec!["assignee_id".to_owned(), "project_id".to_owned(), "reporter_id".to_owned(), "status".to_owned(), "ticket_id".to_owned(), "title".to_owned()]
            || response.rows.len() > 200usize
        { return Err(ApplicationClientError::InvalidResponse); }
        let mut values = Vec::with_capacity(response.rows.len());
        for row in response.rows {
            let [value_0, value_1, value_2, value_3, value_4, value_5] = <[_; 6]>::try_from(row.values).map_err(|_| ApplicationClientError::InvalidResponse)?;
            values.push(BoardPage200FoundTickets {
                assignee_id: decode_wire_uuid(value_0).map_err(|_| ApplicationClientError::InvalidResponse)?,
                project_id: decode_wire_uuid(value_1).map_err(|_| ApplicationClientError::InvalidResponse)?,
                reporter_id: decode_wire_uuid(value_2).map_err(|_| ApplicationClientError::InvalidResponse)?,
                status: if let Some(WireKind::EnumValue(value)) = value_3.kind { match (value.type_id, value.variant_id, value.name.as_str()) { (1, 1, "Open") => "Open".to_owned(), (1, 2, "Closed") => "Closed".to_owned(), (1, 3, "InProgress") => "InProgress".to_owned(), _ => { return Err(ApplicationClientError::InvalidResponse); } } } else { return Err(ApplicationClientError::InvalidResponse); },
                ticket_id: decode_wire_uuid(value_4).map_err(|_| ApplicationClientError::InvalidResponse)?,
                title: if let Some(WireKind::StringValue(value)) = value_5.kind { if value.len() > 128usize { return Err(ApplicationClientError::InvalidResponse); } value } else { return Err(ApplicationClientError::InvalidResponse); },
            });
        }
        Ok(BoardPage200Result::Found(Box::new(BoardPage200Found { tickets: values })))
    }
}
...
pub const BOARD_PAGE450_QUERY_PLAN_HASH: [u8; 32] = [0x22, 0xf1, 0x05, 0xb9, 0x40, 0x37, 0xc2, 0xa8, 0xb4, 0x68, 0x78, 0x13, 0x90, 0x28, 0xe3, 0xee, 0x49, 0x5b, 0x2a, 0xc4, 0xe9, 0x44, 0x95, 0xd2, 0x8b, 0xa2, 0x02, 0xc2, 0x38, 0x6e, 0x8c, 0x84];
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl GeneratedQuery for BoardPage450Query {
    ...
    fn decode_compact_result(outcome: String, response: app_v1::CompactResultField) -> Result<Self::Output, ApplicationClientError> {
        if outcome != "Found"
            ...
            || response.entity != "Ticket"
            || response.fields != vec!["assignee_id".to_owned(), "project_id".to_owned(), "reporter_id".to_owned(), "status".to_owned(), "ticket_id".to_owned(), "title".to_owned()]
            || response.rows.len() > 450usize
        { return Err(ApplicationClientError::InvalidResponse); }
        let mut values = Vec::with_capacity(response.rows.len());
        for row in response.rows {
            let [value_0, value_1, value_2, value_3, value_4, value_5] = <[_; 6]>::try_from(row.values).map_err(|_| ApplicationClientError::InvalidResponse)?;
            values.push(BoardPage450FoundTickets {
                assignee_id: decode_wire_uuid(value_0).map_err(|_| ApplicationClientError::InvalidResponse)?,
                project_id: decode_wire_uuid(value_1).map_err(|_| ApplicationClientError::InvalidResponse)?,
                reporter_id: decode_wire_uuid(value_2).map_err(|_| ApplicationClientError::InvalidResponse)?,
                status: if let Some(WireKind::EnumValue(value)) = value_3.kind { match (value.type_id, value.variant_id, value.name.as_str()) { (1, 1, "Open") => "Open".to_owned(), (1, 2, "Closed") => "Closed".to_owned(), (1, 3, "InProgress") => "InProgress".to_owned(), _ => { return Err(ApplicationClientError::InvalidResponse); } } } else { return Err(ApplicationClientError::InvalidResponse); },
        ...
        Ok(BoardPage450Result::Found(Box::new(BoardPage450Found { tickets: values })))
    }
}
...
pub const BOARD_PAGE50_QUERY_PLAN_HASH: [u8; 32] = [0x1f, 0xe0, 0x70, 0x42, 0x9b, 0x28, 0xa2, 0x87, 0x0a, 0x0f, 0xfd, 0xe2, 0xe0, 0x89, 0xff, 0x0e, 0x79, 0xfd, 0xf7, 0x40, 0xc6, 0x02, 0x52, 0x8f, 0xf3, 0x56, 0x2b, 0xb4, 0x12, 0x3d, 0x11, 0xf7];
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl GeneratedQuery for BoardPage50Query {
    ...
    fn decode_compact_result(outcome: String, response: app_v1::CompactResultField) -> Result<Self::Output, ApplicationClientError> {
        if outcome != "Found"
            ...
            || response.entity != "Ticket"
            || response.fields != vec!["assignee_id".to_owned(), "project_id".to_owned(), "reporter_id".to_owned(), "status".to_owned(), "ticket_id".to_owned(), "title".to_owned()]
            || response.rows.len() > 50usize
        { return Err(ApplicationClientError::InvalidResponse); }
        let mut values = Vec::with_capacity(response.rows.len());
        for row in response.rows {
            let [value_0, value_1, value_2, value_3, value_4, value_5] = <[_; 6]>::try_from(row.values).map_err(|_| ApplicationClientError::InvalidResponse)?;
            values.push(BoardPage50FoundTickets {
                assignee_id: decode_wire_uuid(value_0).map_err(|_| ApplicationClientError::InvalidResponse)?,
                project_id: decode_wire_uuid(value_1).map_err(|_| ApplicationClientError::InvalidResponse)?,
                reporter_id: decode_wire_uuid(value_2).map_err(|_| ApplicationClientError::InvalidResponse)?,
                status: if let Some(WireKind::EnumValue(value)) = value_3.kind { match (value.type_id, value.variant_id, value.name.as_str()) { (1, 1, "Open") => "Open".to_owned(), (1, 2, "Closed") => "Closed".to_owned(), (1, 3, "InProgress") => "InProgress".to_owned(), _ => { return Err(ApplicationClientError::InvalidResponse); } } } else { return Err(ApplicationClientError::InvalidResponse); },
        ...
        Ok(BoardPage50Result::Found(Box::new(BoardPage50Found { tickets: values })))
    }
}
...
pub const GET_TICKET_QUERY_PLAN_HASH: [u8; 32] = [0x14, 0xcf, 0x60, 0x45, 0xdf, 0x12, 0xce, 0xa4, 0xc1, 0x68, 0x00, 0x74, 0xba, 0x99, 0x09, 0x0d, 0x8d, 0xfb, 0xe8, 0xda, 0x8e, 0x02, 0xff, 0x77, 0x8e, 0x82, 0xc8, 0xe8, 0xdc, 0xdf, 0x1d, 0xd7];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const GET_USER_QUERY_PLAN_HASH: [u8; 32] = [0x16, 0x23, 0x69, 0x0c, 0x03, 0x86, 0x5d, 0xab, 0xa8, 0xc6, 0xa4, 0x2a, 0xed, 0x6a, 0xc2, 0x76, 0xe9, 0x65, 0x95, 0x54, 0x1a, 0x2d, 0x5c, 0xd1, 0xc3, 0x54, 0x0e, 0x1d, 0x71, 0x46, 0x62, 0x84];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const PROJECT_MEMBERS_QUERY_PLAN_HASH: [u8; 32] = [0x63, 0x27, 0xa0, 0xd1, 0x5a, 0x75, 0x7c, 0x3f, 0x26, 0x5b, 0x59, 0x5b, 0xcc, 0xf6, 0x97, 0x01, 0x96, 0x50, 0xf5, 0xf2, 0x1e, 0x7e, 0x30, 0xf8, 0x36, 0x6f, 0xb1, 0x81, 0xa4, 0x35, 0x26, 0x69];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const PROJECT_SUMMARY_QUERY_PLAN_HASH: [u8; 32] = [0xf5, 0x8b, 0x2f, 0x9e, 0xdf, 0x69, 0xa8, 0xf0, 0xed, 0xd4, 0x3a, 0x93, 0x63, 0xce, 0x5b, 0xff, 0x3c, 0xd9, 0x08, 0xc3, 0xbb, 0x7b, 0x37, 0xa3, 0xcd, 0xfd, 0xcf, 0x48, 0x71, 0x95, 0x10, 0xe3];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const TICKET_PAGE_QUERY_PLAN_HASH: [u8; 32] = [0xab, 0xbf, 0x37, 0x5e, 0x3d, 0x29, 0x20, 0xad, 0x04, 0x99, 0x13, 0x08, 0xea, 0xa6, 0xff, 0xe4, 0xf8, 0xd2, 0xce, 0x3e, 0x3f, 0xb5, 0x78, 0x05, 0x97, 0x8e, 0x5e, 0x64, 0xde, 0x79, 0x03, 0xf0];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const TICKET_PAGE_PAGED_QUERY_PLAN_HASH: [u8; 32] = [0x10, 0x5d, 0xb7, 0x55, 0xbe, 0x54, 0xdc, 0x65, 0x7c, 0xa7, 0x2c, 0x51, 0xf6, 0xf3, 0xf2, 0x9d, 0xf8, 0xe5, 0x43, 0xbc, 0x3b, 0x4f, 0xfa, 0x6e, 0x04, 0xdb, 0x7a, 0x33, 0x29, 0xca, 0x7a, 0xaf];
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub const TICKET_QUEUE_QUERY_PLAN_HASH: [u8; 32] = [0xec, 0xdc, 0xd9, 0xea, 0x75, 0xfc, 0x9a, 0x1d, 0x76, 0x2b, 0x94, 0xdb, 0xf9, 0x1a, 0x5d, 0x05, 0xcb, 0xce, 0xc1, 0x26, 0xd4, 0xdb, 0xd2, 0xae, 0x31, 0x56, 0x74, 0xad, 0x79, 0xa9, 0xe1, 0xae];
#[derive(Clone, Debug, Eq, PartialEq)]
...
const ADD_PROJECT_MEMBER_PLAN_HASH: [u8; 32] = [0xf6, 0xe9, 0x7f, 0x3c, 0x3d, 0x3e, 0xb0, 0x5a, 0xef, 0x23, 0x0e, 0x22, 0x91, 0x0a, 0x65, 0xff, 0x8d, 0x6b, 0xbb, 0xbc, 0xec, 0xb7, 0x78, 0xc9, 0x31, 0xe8, 0xfb, 0x45, 0x80, 0x5b, 0xba, 0xff];
...
const ATTACH_LABEL_PLAN_HASH: [u8; 32] = [0x7f, 0xa9, 0x1c, 0xe9, 0x49, 0x31, 0x7c, 0xdd, 0xf0, 0x16, 0x4f, 0x98, 0xf4, 0x63, 0x34, 0x4e, 0x90, 0x32, 0xcf, 0xff, 0x69, 0x52, 0x91, 0x98, 0x24, 0xf0, 0xc5, 0x5c, 0xd7, 0x12, 0x40, 0x91];
...
const CLOSE_TICKET_WITH_COMMENT_PLAN_HASH: [u8; 32] = [0x43, 0xbe, 0x33, 0xbb, 0xa0, 0x5e, 0x12, 0x3f, 0x64, 0x97, 0xb4, 0x32, 0x17, 0x4d, 0x8c, 0x27, 0x5e, 0x5c, 0xc3, 0x2b, 0x6c, 0xb6, 0x04, 0x84, 0x9c, 0x40, 0x7f, 0x4b, 0x9c, 0xb7, 0x44, 0xb8];
...
const CREATE_COMMENT_PLAN_HASH: [u8; 32] = [0xfe, 0x01, 0xd1, 0xe9, 0x9d, 0x6c, 0xaf, 0xee, 0xbe, 0x24, 0xb0, 0xbb, 0xe8, 0xf3, 0x69, 0xed, 0x69, 0xed, 0xa4, 0x63, 0xa3, 0xc5, 0xbe, 0xcb, 0xc5, 0x08, 0x60, 0xb7, 0xa6, 0xdc, 0x49, 0x87];
...
const CREATE_PROJECT_PLAN_HASH: [u8; 32] = [0x52, 0xbb, 0x2b, 0xe3, 0x7e, 0x35, 0x7b, 0xf3, 0x8f, 0x97, 0x98, 0x45, 0xdc, 0xba, 0xb0, 0xf6, 0x53, 0x33, 0x20, 0x2c, 0xad, 0x3f, 0xdd, 0xb8, 0xeb, 0x70, 0x19, 0x2a, 0x38, 0x4e, 0x55, 0xf6];
...
const CREATE_TICKET_PLAN_HASH: [u8; 32] = [0xdb, 0xc8, 0x7a, 0x37, 0x36, 0xe4, 0xac, 0xb0, 0x9d, 0xd9, 0x52, 0x47, 0xdd, 0x65, 0x98, 0xb5, 0xad, 0xf9, 0x13, 0x1b, 0x0c, 0x9c, 0xe5, 0xbb, 0x88, 0xb1, 0xf8, 0x2b, 0x84, 0xed, 0xe0, 0x69];
...
const CREATE_USER_PLAN_HASH: [u8; 32] = [0xa1, 0xe6, 0x7b, 0x10, 0xa1, 0x00, 0x27, 0x72, 0x8f, 0x41, 0x2f, 0xbb, 0x06, 0x5e, 0x43, 0x1e, 0x0b, 0x9c, 0xc6, 0x75, 0x26, 0xb2, 0x3f, 0x9d, 0x83, 0x32, 0x52, 0x13, 0xd8, 0x0a, 0x85, 0xa2];
...
const OPEN_TICKET_WITH_LABELS_PLAN_HASH: [u8; 32] = [0xb5, 0x2f, 0x08, 0x0b, 0x84, 0xac, 0x70, 0xd7, 0x7c, 0x50, 0x5b, 0xf2, 0xcc, 0x9d, 0xa2, 0x58, 0xb6, 0x43, 0x4c, 0x1f, 0x97, 0x88, 0xf3, 0xd1, 0x5b, 0xbb, 0x65, 0x8a, 0x70, 0x85, 0x7c, 0x9e];
...
const SWAP_MEMBER_ROLES_PLAN_HASH: [u8; 32] = [0x9b, 0xe1, 0x78, 0x7d, 0xa3, 0x66, 0x0f, 0x0c, 0x4d, 0x9a, 0xb9, 0x75, 0x09, 0x97, 0x9b, 0xc9, 0x46, 0x6b, 0x49, 0xf3, 0x87, 0x62, 0xee, 0x74, 0x7c, 0x3c, 0x1e, 0xe2, 0x1c, 0x25, 0x26, 0x95];
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TicketEventsConsumer {
    pub parameters: TicketEventsParams,
    pub consumer_name: String,
}
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TicketEventsTicketCreated {
    pub assignee_id: String,
    pub organization_id: String,
    pub project_id: String,
    pub reporter_id: String,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TicketEventsEvent {
    TicketCreated(TicketEventsTicketCreated),
}
pub type TicketEventsDelivery = riffdb_client_rust::TypedEventDelivery<TicketEventsEvent>;
impl GeneratedEventConsumer for TicketEventsConsumer {
    type Event = TicketEventsEvent;
    fn event_consumer(self) -> Result<ApplicationEventConsumer, ApplicationClientError> {
        ...
        ApplicationEventConsumer::new(ApplicationReactiveOperation::new(TICKET_ACTIVITY_REACTIVE_MODULE_HASH, "TicketEvents", parameters)?, self.consumer_name)
    }
    fn decode_event(event: ApplicationEvent) -> Result<Self::Event, ApplicationClientError> {
        ...
        let decoded = match event.name.as_str() {
            "TicketCreated" => TicketEventsEvent::TicketCreated(TicketEventsTicketCreated {
                assignee_id: application_uuid(take_application_value(&mut fields, "assignee_id")?)?,
                organization_id: application_uuid(take_application_value(&mut fields, "organization_id")?)?,
                project_id: application_uuid(take_application_value(&mut fields, "project_id")?)?,
                reporter_id: application_uuid(take_application_value(&mut fields, "reporter_id")?)?,
                status: application_enum(take_application_value(&mut fields, "status")?)?,
                ticket_id: application_uuid(take_application_value(&mut fields, "ticket_id")?)?,
                title: application_string(take_application_value(&mut fields, "title")?)?,
            }),
...
impl TicketDeskClient {
    pub async fn next_ticket_events(&mut self, consumer: TicketEventsConsumer, options: EventConsumerOptions) -> Result<TypedEventBatch<TicketEventsEvent>, ApplicationClientError> {
        self.client.consume_generated_events(consumer, options, &self.metadata).await
    }
    pub async fn ack_ticket_events(&mut self, consumer: &TicketEventsConsumer, delivery: &TicketEventsDelivery) -> Result<ApplicationEventMutationResult, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
        self.client.acknowledge_event(&identity, delivery.evidence(), &self.metadata).await
    }
    pub async fn nack_ticket_events(&mut self, consumer: &TicketEventsConsumer, delivery: &TicketEventsDelivery, retry_delay_nanos: u64) -> Result<ApplicationEventMutationResult, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
        self.client.negative_acknowledge_event(&identity, delivery.evidence(), retry_delay_nanos, &self.metadata).await
    }
    pub async fn seek_ticket_events(&mut self, consumer: &TicketEventsConsumer, checkpoint: ApplicationEventCheckpoint) -> Result<ApplicationEventMutationResult, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TicketPageWatchParams {
    pub organization_id: String,
    pub ticket_id: String,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TicketPageWatchUpdate {
    Snapshot(TypedLiveQuerySnapshot<TicketPageResult>),
    Patch(riffdb_client_rust::LiveQueryPatch),
    Reset(TypedLiveQueryReset<TicketPageResult>),
    Checkpoint(riffdb_client_rust::LiveQueryCheckpoint),
    Terminal(riffdb_client_rust::LiveQueryTerminal),
}
impl GeneratedLiveQuery for TicketPageWatchParams {
    type Update = TicketPageWatchUpdate;
    fn live_operation(self) -> Result<ApplicationReactiveOperation, ApplicationClientError> {
        ...
        parameters.insert("ticket_id".to_owned(), ApplicationValue::Uuid(ApplicationUuid::from_text(self.ticket_id)?));
        ApplicationReactiveOperation::new(TICKET_ACTIVITY_REACTIVE_MODULE_HASH, "TicketPageWatch", parameters)
    }
    fn decode_update(update: ApplicationLiveQueryUpdate) -> Result<Self::Update, ApplicationClientError> {
        match update {
            ApplicationLiveQueryUpdate::Snapshot { result, cursor, history_incarnation, application_head } => Ok(TicketPageWatchUpdate::Snapshot(TypedLiveQuerySnapshot { result: <TicketPageQuery as GeneratedQuery>::decode_result(result)?, checkpoint: LiveQueryCheckpoint { history_incarnation, application_head, cursor } })),
            ApplicationLiveQueryUpdate::Patch(value) => Ok(TicketPageWatchUpdate::Patch(value)),
            ApplicationLiveQueryUpdate::Reset { reason, result, cursor, history_incarnation, application_head } => Ok(TicketPageWatchUpdate::Reset(TypedLiveQueryReset { reason, result: <TicketPageQuery as GeneratedQuery>::decode_result(result)?, checkpoint: LiveQueryCheckpoint { history_incarnation, application_head, cursor } })),
            ApplicationLiveQueryUpdate::Checkpoint(value) => Ok(TicketPageWatchUpdate::Checkpoint(value)),
            ApplicationLiveQueryUpdate::Terminal(value) => Ok(TicketPageWatchUpdate::Terminal(value)),
        }
    }
}
impl TicketDeskClient {
    pub async fn watch_ticket_page_watch(&mut self, parameters: TicketPageWatchParams, cursor: Option<LiveQueryCursor>) -> Result<TypedLiveQueryStream<TicketPageWatchParams>, ApplicationClientError> {
        self.client.watch_generated_query(parameters, cursor, &self.metadata).await
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TicketQueueWatchParams {
    pub organization_id: String,
    pub project_id: String,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TicketQueueWatchUpdate {
    Snapshot(TypedLiveQuerySnapshot<TicketQueueResult>),
    Patch(riffdb_client_rust::LiveQueryPatch),
    Reset(TypedLiveQueryReset<TicketQueueResult>),
    Checkpoint(riffdb_client_rust::LiveQueryCheckpoint),
    Terminal(riffdb_client_rust::LiveQueryTerminal),
}
impl GeneratedLiveQuery for TicketQueueWatchParams {
    type Update = TicketQueueWatchUpdate;
    fn live_operation(self) -> Result<ApplicationReactiveOperation, ApplicationClientError> {
        let mut parameters = BTreeMap::new();
        parameters.insert("organization_id".to_owned(), ApplicationValue::Uuid(ApplicationUuid::from_text(self.organization_id)?));
        parameters.insert("project_id".to_owned(), ApplicationValue::Uuid(ApplicationUuid::from_text(self.project_id)?));
        ApplicationReactiveOperation::new(TICKET_ACTIVITY_REACTIVE_MODULE_HASH, "TicketQueueWatch", parameters)
    }
    fn decode_update(update: ApplicationLiveQueryUpdate) -> Result<Self::Update, ApplicationClientError> {
        match update {
            ApplicationLiveQueryUpdate::Snapshot { result, cursor, history_incarnation, application_head } => Ok(TicketQueueWatchUpdate::Snapshot(TypedLiveQuerySnapshot { result: <TicketQueueQuery as GeneratedQuery>::decode_result(result)?, checkpoint: LiveQueryCheckpoint { history_incarnation, application_head, cursor } })),
            ApplicationLiveQueryUpdate::Patch(value) => Ok(TicketQueueWatchUpdate::Patch(value)),
            ApplicationLiveQueryUpdate::Reset { reason, result, cursor, history_incarnation, application_head } => Ok(TicketQueueWatchUpdate::Reset(TypedLiveQueryReset { reason, result: <TicketQueueQuery as GeneratedQuery>::decode_result(result)?, checkpoint: LiveQueryCheckpoint { history_incarnation, application_head, cursor } })),
            ApplicationLiveQueryUpdate::Checkpoint(value) => Ok(TicketQueueWatchUpdate::Checkpoint(value)),
            ApplicationLiveQueryUpdate::Terminal(value) => Ok(TicketQueueWatchUpdate::Terminal(value)),
        }
    }
}
impl TicketDeskClient {
    pub async fn watch_ticket_queue_watch(&mut self, parameters: TicketQueueWatchParams, cursor: Option<LiveQueryCursor>) -> Result<TypedLiveQueryStream<TicketQueueWatchParams>, ApplicationClientError> {
        self.client.watch_generated_query(parameters, cursor, &self.metadata).await
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TriageTicketParams {
    pub organization_id: String,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TriageTicketConsumer {
    pub parameters: TriageTicketParams,
    pub consumer_name: String,
}
pub type TriageTicketItem = TypedContextualWorkItem<TicketEventsEvent>;
impl GeneratedEventConsumer for TriageTicketConsumer {
    type Event = TicketEventsEvent;
    fn event_consumer(self) -> Result<ApplicationEventConsumer, ApplicationClientError> {
        let mut parameters = BTreeMap::new();
        parameters.insert("organization_id".to_owned(), ApplicationValue::Uuid(ApplicationUuid::from_text(self.parameters.organization_id)?));
        ApplicationEventConsumer::new(ApplicationReactiveOperation::new(TICKET_ACTIVITY_REACTIVE_MODULE_HASH, "TriageTicket", parameters)?, self.consumer_name)
    }
    fn decode_event(event: ApplicationEvent) -> Result<Self::Event, ApplicationClientError> {
        <TicketEventsConsumer as GeneratedEventConsumer>::decode_event(event)
    }
}
impl TicketDeskClient {
    pub async fn next_triage_ticket(&mut self, consumer: TriageTicketConsumer, maximum_wait_nanos: u64) -> Result<TypedContextualBatch<TicketEventsEvent>, ApplicationClientError> {
        self.client.consume_generated_contextual(consumer, maximum_wait_nanos, &self.metadata).await
    }
    pub async fn ack_triage_ticket(&mut self, consumer: &TriageTicketConsumer, item: &TriageTicketItem) -> Result<ApplicationEventMutationResult, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
        self.client.acknowledge_contextual_item(&identity, item.evidence(), &self.metadata).await
    }
    pub async fn nack_triage_ticket(&mut self, consumer: &TriageTicketConsumer, item: &TriageTicketItem, retry_delay_nanos: u64) -> Result<ApplicationEventMutationResult, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
    ...
    pub async fn react_comment(&mut self, consumer: &TriageTicketConsumer, item: &TriageTicketItem, command: &CreateCommentInput) -> Result<TypedCommandResult<CreateCommentOutcome>, ApplicationClientError> {
        let identity = consumer.clone().event_consumer()?;
        let reaction = item.evidence().available_reactions.iter().find(|reaction| reaction.name == "comment" && reaction.command_name == "CreateComment").ok_or(ApplicationClientError::InvalidResponse)?;
