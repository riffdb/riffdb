...
From version 1.30 this table is closed. Later changes are recorded in git history and in the front matter of the ADR that decides them; a change to the text of a requirement still edits that requirement in place.

| Version | Date | Summary |
|---|---|---|
| 1.30 | 2026-09-02 | Process cutover to governance v2. Every change now carries a tier that `./scripts/classify-change` computes from its paths against `governance/tiers.yaml`: an internal change requires tests and acceptance only, a surface change requires a short-form ADR v2 record and human review of the fixture diffs, and a guarantee change requires a full record whose exact text a human accepts before merge. An ADR v2 record opens with YAML front matter that owns its tier, requirements, packages, obligations, and review triggers, and the `adr/README.md` index is generated by `./scripts/adr-index --write`; records ADR-0001 through ADR-0177 keep the legacy header and are read as accepted. A requirement is proven by a `// req:` or `# req:` tag on the test that establishes it, an ADR obligation's proof MUST exist in code rather than in prose, and `adr/obligations-outstanding.yaml` is closed to new entries and only shrinks. `./scripts/acceptance` runs the path-triggered plan for one change, `./scripts/ci-all` remains the battery at merge, and `./scripts/brief WP-NNN` renders the package prompt. Section 19.6, the required CI gates of section 18, and every machine check are unchanged, as are all durable formats, storage keys, protocols, languages, and provider algorithms. |
...
The maintainer accepted the exact text of
`docs/architecture/WP-749-ARCHIVE-RESTORE-RECEIPT-REVIEW.md` in session on
2026-09-15: "Approve exact text". The following qualifies ADR-0050's restore
input, RPC inventory and V1-only writer rules and completes ADR-0178 §7.
WP-749 owns implementation and the required compatibility and recovery proofs.
...
| Governance | `classify-change`, `check-requirement-coverage`, `check-adr-obligations`, `adr-index --check` |
...
Every PR MUST contain six fields:
...
Run `./scripts/brief WP-NNN`; the brief is the prompt. It renders the objective, tier, dependencies and their closure status, allowed paths, deliverables, exit gate, standing design tests, the requirement text from this specification, the decisions of the required ADRs, the obligations the package owns, the acceptance plan, and the human review triggers of section 19.6.
