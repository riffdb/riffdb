...
Acceptance is a human act: a human sets `status: accepted` and the `accepted:`
date. Exact-text acceptance before merge is required at guarantee tier. A
surface record is accepted with the change it describes. Direction approval is
