...
| Staleness tracking and observer-backed SLO health (VEC-003/VEC-004/VEC-012) | IN PROGRESS | One canonical evidence classification now defines missing-embedding and source-newer staleness, and a separately versioned authoritative observation record maintains bounded total, stale, and exact-model counts in the reference transaction path. The production redb table, ordered inspection indexes, global threshold observer, and `HealthComponentKind::VectorStaleness` wiring remain pending. Health remains deliberately absent until those production pieces are atomic and validated; the in-progress counter format is not observer proof. |
...
production nearest adapter, and accepted
