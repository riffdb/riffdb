...
| Declared exact/ANN threshold routing (VEC-009) | PROVEN | `uncached_declared_threshold_queries_remain_exact` and `cold_exact_warm_reuse_and_recall_at_the_production_bound` prove ADR-0229 cold exact fallback and warm ANN above the compiler-owned threshold |
| Declared recall at matched frontiers (VEC-010) | PROVEN | `cold_exact_warm_reuse_and_recall_at_the_production_bound` and `hnsw::tests::randomized_histories_meet_declared_recall_against_exact_at_matched_frontiers` compare ANN recall@K with the exact reference over bounded seeded histories at identical visible frontiers |
| Per-org ANN statistics and policy isolation (VEC-007/VEC-008) | PROVEN | `another_organization_cannot_change_results_or_graph_statistics` proves 512 rows in another org change neither graph stats, work, distances, nor results; `denied_malformed_rows_and_other_orgs_never_enter_reused_graphs` proves denied rows cannot influence warm graphs while still consuming scan work |
...
| Derived/non-authoritative lifecycle, source selection, freshness, and replay-budget detach for nearest (VEC-005) | PROVEN | RiffQL V5/query IR V8/module V8 bind one exact `Entity.field` source and `Available`, causal, or duration-bounded freshness. The server automatically registers that source (`production_vector_contract_registers_exact_symbolic_projection_automatically`), causal reads wait through build/rebuild using register-before-observe, and bounded freshness uses trusted commit timestamps (`bounded_vector_freshness_uses_elapsed_time_across_second_boundaries`). Durable vector controls attach retention only while `Ready`; declared age/byte/backlog breaches allocate a successor generation; stable authoritative snapshot plus retained tail builds privately; checkpoint precedes Ready publication. `persisted_vector_rebuild_resumes_after_storage_reopen_without_overclaim`, `detached_and_rebuilding_vector_controls_resume_without_frontier_overclaim`, and the redb control/retention suites pin restart, generation identity, and no-overclaim behavior. The real-process generated-client test exercises the same lifecycle before exact release. |
| Authoritative embedding and model-version production (VEC-002/VEC-003) | PROVEN | Contract IR V15 carries a compiler-sealed `SetEmbedding`; service validation rejects wrong model identity/version at exact symbolic input paths; runtime emits one bounded evidence intent; commit re-derives transaction-current evidence transitions; memory/redb persist entity state plus evidence atomically; all four generated languages expose canonical vectors with declared-model constructors; and generated clients expose the exact symbolic staleness/model operations. |
| Staleness tracking and observer-backed SLO health (VEC-003/VEC-004/VEC-012) | PROVEN | Redb maintains the authoritative evidence row, ordered inspection index, per-partition observation, and global threshold observation atomically. `vector_inspection_reads_counts_evidence_and_frontier_from_one_snapshot` proves one exact read snapshot; startup fixtures prove registry installation/backfill before publication; `vector_health_is_present_and_fails_closed_by_observer_state` proves healthy/degraded/unavailable behavior without a probe scan. `remote_generated_vector_client_writes_inspects_and_queries_exact_projection` proves the public role-bound grant, generated inspection, and authenticated health response against real `riffdbd`. |
| Frontier contract for production vector nearest (VEC-011) | PROVEN | Production results bind the selected published frontier, and RiffQL declares `Causal`, duration-`Bounded`, or `Available`. Generic lifecycle tests prove register-before-observe causal waiting, timeout, revocation, and bounded behavior; vector-specific source registration and elapsed-time arithmetic are pinned. The real-process generated-client query fences its exact nearest read to the embedding command commit while build/rebuild remains private. |
| Complete application ingress for embeddings (VEC-002) | PROVEN | The native value, Protobuf field 15, generated gRPC boundary, hosted MCP conversion, CLI conversion, compiler-sealed `embed` effect, atomic authoritative evidence persistence, shared driver value registry, and generated Rust/Go/TypeScript/Python vector models are reachable and bounded. `remote_generated_vector_client_writes_inspects_and_queries_exact_projection` writes canonical vector data through a generator-owned Rust client against real `riffdbd`. |
| Production nearest-query reachability | PROVEN | The API-neutral service and server join one compiled V8 source to the exact columnar engine, authoritative evidence/current-model admission, scalar filters, K, freshness, and pre-release policy revalidation. Source-less retained plans fail with `RDB-PROJECTION-0104`; mixed bindings and mismatched sources fail at compile time. The real-process generated-client test commits an embedding and receives the exact fenced nearest result through public gRPC. |
...
row against named production evidence. Implemented pieces remain fail closed when their exact source,
...
embedding/model evidence, observer-backed staleness, symbolic inspection, the
exact production nearest adapter, and durable generation control with
replay-budget detach, private snapshot-plus-tail rebuild, Ready-only retention,
and reopen/no-overclaim coverage. Missing or invalid observer state still
reports `vector_staleness: unavailable`, and source-less retained plans fail
with `RDB-PROJECTION-0104`. The generator-owned real-process test now proves
write, role-bound inspection, health, and exact causal nearest through the
public client; restart and no-overclaim remain independently pinned by the
durable control tests rather than inferred from that happy path.
