...
| One-time secret handout | **VERIFIED** | `IssueVerificationToken` hands out the secret-classified digest exactly once through `token.token_digest reveals token.token_digest`. `token_handout_flow_carries_the_exact_static_secret_reveal` proves the annotation names that one direct source and is present in V11 executable plan identity; the compiler's WP-600 negative fixtures prove it cannot be omitted, widened, or supplied at runtime. |
...
| Refresh/revoke race through the commit coordinator | **VERIFIED** | WP-606 audited and admitted each required executable version independently. `framework_refresh_wins_the_race_and_revocation_observes_its_declared_stale_outcome` and `framework_revocation_wins_the_race_and_refresh_observes_its_declared_stale_outcome` now run live through the real coordinator in both submission orders, pin the loser replay and final state, and retain the beyond-audited-version refusal. |
...
- The one intentional secret disclosure (issuance handout) is a single
  source-naming `reveals` flow site carried in V11 plan identity; removing or
  widening the declaration fails compilation.
