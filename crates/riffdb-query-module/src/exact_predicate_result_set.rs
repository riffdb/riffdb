...
use riffdb_query_compiler::{CompiledExactPredicateQueryV1, CompiledExactPredicateQueryV2};
use riffdb_query_ir::{
    AuthorizationEntityAccess, ExactPredicateProgramV1, ExactPredicateProgramV2, NamedQuerySchemas,
...
const VERSION: u16 = 1;
const NULLABLE_MAGIC: &[u8] = b"RIFFDB-NULLABLE-EXACT-PREDICATE-QUERY\0";
const NULLABLE_VERSION: u16 = 1;
...
/// Immutable query-module binding for one nullable exact family.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CompiledNullableExactPredicateResultSetV1 {
    name: QueryOperationName,
...
impl CompiledNullableExactPredicateResultSetV1 {
    /// Seals compiler output into one canonical V10 module artifact.
    pub fn new(
        name: QueryOperationName,
        compiled: CompiledExactPredicateQueryV2,
        ...
        canonical_bytes.extend_from_slice(NULLABLE_MAGIC);
        canonical_bytes.extend_from_slice(&NULLABLE_VERSION.to_be_bytes());
    ...
    /// Provider-independent nullable predicate/order program.
    #[must_use]
    ...
    /// Request-time authorization charge after background work is separated.
    #[must_use]
