...
/// Additive module codec for explicitly bounded finite partition-set routes.
...
pub enum CompiledNamedQueryPlan {
    ...
    /// Exact bounded tokenized-text membership over one declared index.
    TokenizedTextV1(Arc<CompiledTokenizedTextResultSetV1>),
}
...
impl CompiledNamedQueryPlan {
    ...
    pub fn schemas(&self) -> &NamedQuerySchemas {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.schemas(),
        }
    }
    ...
    pub fn secret_outputs(&self) -> &[SecretOutputRequirement] {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.secret_outputs(),
        }
    }
    ...
    pub fn identity(&self) -> QueryPlanHash {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.identity(),
        }
    }
    ...
    pub fn authorization(&self) -> &[AuthorizationEntityAccess] {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.authorization(),
        }
    }
    ...
    pub fn cost(&self) -> QueryCostVectorV1 {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.cost(),
        }
    }
    ...
    pub fn authorization_cost(&self) -> QueryCostVectorV1 {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.authorization_cost(),
            _ => self.cost(),
        }
    }
    ...
    pub fn canonical_bytes(&self) -> &[u8] {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.canonical_bytes(),
        }
    }
    ...
    pub fn representative_program(&self) -> &QueryAccessProgramV1 {
        match self {
            ...
            Self::TokenizedTextV1(tokenized) => tokenized.representative_program(),
        }
    }
}
...
impl CompiledNamedQuery {
    ...
    /// Exact predicate/order family, when this is an ADR-0134 operation.
    #[must_use]
    ...
    /// Compiler-sealed tokenized-text operation, when present.
    #[must_use]
    pub fn tokenized_text_result(&self) -> Option<&CompiledTokenizedTextResultSetV1> {
        match &self.plan {
    ...
    /// Shared immutable tokenized-text plan for derived-provider execution.
    #[doc(hidden)]
    #[must_use]
    pub fn shared_tokenized_text_result(&self) -> Option<Arc<CompiledTokenizedTextResultSetV1>> {
        match &self.plan {
...
fn compile_document_plan(
    ...
    if query_shape_version == RIFFQL_LANGUAGE_VERSION_TOKENIZED_TEXT_V1 {
        let compiled = compile_tokenized_text_query_v1(document, catalog)?;
        ...
        CompiledTokenizedTextResultSetV1::new(operation, compiled)
            .map(|tokenized| CompiledNamedQueryPlan::TokenizedTextV1(Arc::new(tokenized)))
            .map_err(|_| {
                exact_text_artifact_invariant(
                    document
                        .name
    ...
    } else if query_shape_version == RIFFQL_LANGUAGE_VERSION_NULLABLE_EXACT_ORDER_V1 {
        ...
        CompiledExactPredicateResultSetV1::new(name, compiled)
            .map(|exact| CompiledNamedQueryPlan::ExactPredicateV1(Arc::new(exact)))
            .map_err(|_| {
                exact_predicate_artifact_invariant(
                    document
                        .name
...
fn query_source_map(query: &CompiledNamedQuery) -> &QuerySourceMap {
    match query.plan() {
        ...
        CompiledNamedQueryPlan::TokenizedTextV1(tokenized) => tokenized.source_map(),
    }
}
...
fn query_explain_lines(query: &CompiledNamedQuery) -> Vec<String> {
    let mut lines = match query.plan() {
        ...
        CompiledNamedQueryPlan::ExactPredicateV1(exact) => vec![
            format!(
                "exact_predicate.members={}",
                exact.program().members().len()
            ),
            format!(
                "exact_predicate.presence_parameters={}",
                exact.presence_parameters().join(",")
            ),
            format!(
                "exact_predicate.value_parameters={}",
                exact.value_parameters().join(",")
            ),
            format!(
                "exact_predicate.limit_parameter=${}",
                exact.limit_parameter()
            ),
            format!(
                "exact_predicate.offset_parameter=${}",
        ...
        CompiledNamedQueryPlan::TokenizedTextV1(tokenized) => vec![
            format!(
                "tokenized_text.index={}",
                tokenized.tokenized_plan().index().get()
            ),
            format!(
                "tokenized_text.analyzer={}",
                tokenized.tokenized_plan().analyzer().as_str()
            ),
            format!(
                "tokenized_text.match={:?}",
                tokenized.tokenized_plan().kind()
            ),
            format!(
                "tokenized_text.query_parameter=${}",
                tokenized.query_parameter()
            ),
            format!(
                "tokenized_text.max_terms={}",
                tokenized.tokenized_plan().max_terms()
            ),
            format!(
                "tokenized_text.max_candidates={}",
                tokenized.tokenized_plan().max_candidates()
            ),
            format!(
                "tokenized_text.max_results={}",
                tokenized.tokenized_plan().max_results()
            ),
            "tokenized_text.provider=compiler_required".to_owned(),
        ],
    };
...
fn encode_module(
    ...
    let partition_set = queries.iter().any(|query| {
        query
            .plan()
            .representative_program()
            .surface()
    ...
    let format_version = if partition_set {
        QUERY_MODULE_FORMAT_VERSION_PARTITION_SET_V1
    ...
    let versions_match = match format_version {
        ...
        QUERY_MODULE_FORMAT_VERSION_PARTITION_SET_V1 => {
            language_version == RIFFQL_LANGUAGE_VERSION_PARTITION_SET_V1
                && ir_version == QUERY_IR_VERSION_PARTITION_SET_V1
        }
        _ => false,
    };
    if !versions_match {
    ...
    for _ in 0..query_count {
        ...
        for _ in 0..input.count(riffdb_query_ir::MAX_SOURCE_MAP_ENTRIES)? {
            ...
            let maximum_tag = match format_version {
                ...
                QUERY_MODULE_FORMAT_VERSION_BOUNDED_RESULT_PIPELINE_V1
                | QUERY_MODULE_FORMAT_VERSION_ORDER_FAMILY_V1
                | QUERY_MODULE_FORMAT_VERSION_PARTITION_SET_V1 => 12,
