...
fn generate_go_client_inner(
    ...
    .infallible();
    writeln!(output, "package {}\n", go_package(module.name().as_str())).infallible();
...
fn emit_entities(output: &mut String, contract: &ContractBundle) {
    for entity in contract.schema().entities() {
        writeln!(output, "type {} struct {{", go_public(entity.name())).infallible();
...
fn emit_query_types(output: &mut String, module: &QueryModule, contract: &ContractBundle) {
    for query in module.queries() {
        ...
        if !query.plan().secret_outputs().is_empty() {
            writeln!(output, "var {name}SecretOutputs = []QuerySecretOutput{{").infallible();
        ...
        writeln!(output, "type {name}Params struct {{").infallible();
        for parameter in query.plan().schemas().parameters() {
            ...
            writeln!(output, "\t{} {}", go_public(parameter.name()), ty).infallible();
        }
        output.push_str("}\n\n");
        writeln!(output, "type {name}Result interface {{ is{name}Result() }}").infallible();
        for branch in query.plan().schemas().results() {
            let branch_name = format!("{name}{}", go_public(branch.name()));
            writeln!(output, "type {branch_name} struct {{\n\tOutcome string").infallible();
...
fn emit_command_types(output: &mut String, contract: &ContractBundle) {
    ...
    for command in commands {
        ...
        if !secret_outputs.is_empty() {
            writeln!(output, "var {name}SecretOutputs = []struct {{ Outcome, Field, Entity, SourceField string }}{{").infallible();
        ...
        writeln!(output, "type {name}Input struct {{").infallible();
...
fn emit_go_embedding_constructors(
    ...
    for facade in facades {
        ...
        .infallible();
        writeln!(output, "type {declared_name} struct {{").infallible();
        ...
        for field in command.input().record().fields() {
            ...
            if field.name() == facade.model_input_name {
                writeln!(output, "\t\t{public}: {identity_constant},").infallible();
            } else if field.name() == facade.version_input_name {
                writeln!(output, "\t\t{public}: {version_constant},").infallible();
            } else {
                writeln!(output, "\t\t{public}: input.{public},").infallible();
            }
        }
        writeln!(
            output,
...
fn emit_entity_codecs(output: &mut String, contract: &ContractBundle) {
    for entity in contract.schema().entities() {
        ...
        writeln!(output, "func encode{name}(value {name}) riffdb.Value {{ return riffdb.Record(map[string]riffdb.Value{{").infallible();
        for field in go_wire_model_fields(entity.record()) {
            writeln!(
                output,
                "\t\"{}\": {},",
        ...
        output.push_str("}) }\n");
        writeln!(output, "func decode{name}(value riffdb.Value) ({name}, error) {{ fields, err := riffdb.RecordFields(value); if err != nil {{ return {name}{{}}, err }}; var result {name}; var raw riffdb.Value").infallible();
        for field in go_wire_model_fields(entity.record()) {
            let public = go_public(field.name());
            writeln!(output, "raw, err = requiredField(fields, \"{}\"); if err != nil {{ return result, err }}; result.{public}, err = {}; if err != nil {{ return result, err }}", field.name(), decode_expr("raw", field.value_type(), contract)).infallible();
        }
...
fn emit_query_methods(
    ...
    for query in module.queries() {
        ...
        .infallible();
        writeln!(output, "var {name}Operation = riffdb.Operation{{Name: \"{operation}\", InputSchemaHash: \"{schema_hash}\"}}").infallible();
        ...
        for branch in query.plan().schemas().results() {
            ...
            for field in branch.fields() {
                let public = go_public(field.name());
                writeln!(output, "raw, err = requiredField(fields, \"{}\"); if err != nil {{ return nil, err }}; result.{public}, err = {}; if err != nil {{ return nil, err }}", field.name(), decode_named_expr("raw", field.value_type(), contract)).infallible();
            }
            output.push_str("return result, nil\n");
        }
        ...
        writeln!(output, "func (client *Client) {name}(ctx context.Context, parameters {name}Params, options QueryOptions) (QueryResult[{name}Result], error) {{ input := map[string]riffdb.Value{{}}").infallible();
        for parameter in query.plan().schemas().parameters() {
...
fn emit_command_methods(
    ...
    for command in commands {
        ...
        .infallible();
        writeln!(output, "var {name}Operation = riffdb.Operation{{Name: \"{operation}\", InputSchemaHash: \"{schema_hash}\"}}").infallible();
        writeln!(output, "func encode{name}Input(input {name}Input) map[string]riffdb.Value {{ return map[string]riffdb.Value{{").infallible();
        for field in command.input().record().fields() {
            writeln!(
                output,
                "\t\"{}\": {},",
        ...
        writeln!(output, "func decode{name}Outcome(value riffdb.Value) ({name}Outcome, error) {{ fields, err := riffdb.RecordFields(value); if err != nil {{ return nil, err }}; outcomeValue, err := requiredField(fields, \"outcome\"); if err != nil {{ return nil, err }}; outcome, err := riffdb.EnumValue(outcomeValue); if err != nil {{ return nil, err }};{raw_declaration} switch outcome {{").infallible();
        for outcome in command.outcomes() {
            let outcome_name = format!("{name}{}", go_public(outcome.name()));
            writeln!(
                output,
            ...
            for field in outcome.payload().fields() {
                let public = go_public(field.name());
                writeln!(output, "raw, err = requiredField(fields, \"{}\"); if err != nil {{ return nil, err }}; result.{public}, err = {}; if err != nil {{ return nil, err }}", field.name(), decode_expr("raw", field.value_type(), contract)).infallible();
            }
            output.push_str("return result, nil\n");
        }
        ...
        if !workflow_revisions.is_empty() {
            writeln!(output, "func workflow{name}Revisions(input {name}Input, outcome {name}Outcome) ([]WorkflowSuccessorRevision, error) {{ if _, ok := outcome.({name}{}); !ok {{ return nil, nil }}", go_public(success_outcome)).infallible();
            for revision in &workflow_revisions {
                writeln!(output, "if input.{} == ^uint64(0) {{ return nil, errors.New(\"RiffDB workflow successor revision overflow\") }}", go_public(revision.input_name)).infallible();
            }
            output.push_str("return []WorkflowSuccessorRevision{");
            for revision in &workflow_revisions {
                write!(
                    output,
                    "{{Binding: {:?}, Revision: input.{} + 1}},",
                    revision.binding_name,
                    go_public(revision.input_name)
                )
                .infallible();
            }
            output.push_str("}, nil }\n");
        }
        let revision_call = if workflow_revisions.is_empty() {
            String::new()
        } else {
            format!(
                "workflowRevisions, err := workflow{name}Revisions(input, outcome); if err != nil {{ return CommandResult[{name}Outcome]{{}}, err }}; "
            )
        };
        let revision_field = if workflow_revisions.is_empty() {
            String::new()
        } else {
            ", WorkflowRevisions: workflowRevisions".to_owned()
        };
        writeln!(output, "func (client *Client) {name}(ctx context.Context, input {name}Input) (CommandResult[{name}Outcome], error) {{ {collection_validation}response, err := client.session.Invoke(ctx, {name}Operation, encode{name}Input(input), riffdb.Options{{MaximumAttempts: client.commandAttempts}}); if err != nil {{ return CommandResult[{name}Outcome]{{}}, err }}; outcome, err := decode{name}Outcome(response.Value); if err != nil {{ return CommandResult[{name}Outcome]{{}}, err }}; {revision_call}return CommandResult[{name}Outcome]{{Outcome: outcome, CommitSequence: response.ApplicationHead, ContractVersion: ContractVersion, PlanHash: {name}PlanHash, Replayed: response.Replayed, OutcomeURI: response.Cursor{revision_field}}}, nil }}").infallible();
        let batch_revision_call = if workflow_revisions.is_empty() {
            String::new()
        } else {
            format!(
                "if item.Index >= uint32(len(inputs)) {{ return BatchResult[{name}Outcome]{{}}, errors.New(\"RiffDB driver returned invalid batch index\") }}; workflowRevisions, revisionErr := workflow{name}Revisions(inputs[item.Index], outcome); if revisionErr != nil {{ return BatchResult[{name}Outcome]{{}}, revisionErr }}; "
            )
        };
        let batch_revision_field = if workflow_revisions.is_empty() {
            String::new()
        } else {
            ", WorkflowRevisions: workflowRevisions".to_owned()
        };
    ...
    for operation in reactive.operations() {
        ...
        match operation.plan() {
            ReactiveOperationPlanV1::Stream {
                ...
                for event in events {
                    writeln!(output, "type {name}{} struct {{", go_public(event.name()))
                        .infallible();
                ...
                writeln!(output, "type {name}Event interface {{ is{name}Event() }}").infallible();
                writeln!(output, "func decode{name}Event(eventName string, fields map[string]riffdb.Value) ({name}Event, error) {{ var raw riffdb.Value; var err error; switch eventName {{").infallible();
                for event in events {
                    ...
                    for field in event.fields() {
                        let public = go_public(field.name());
                        writeln!(output, "raw, err = requiredField(fields, {:?}); if err != nil {{ return nil, err }}; result.{public}, err = {}; if err != nil {{ return nil, err }}", field.name(), go_reactive_decode_expr("raw", field.type_name(), contract)).infallible();
                    }
                    output.push_str("return result, nil\n");
                }
            ...
            ReactiveOperationPlanV1::Subscription {
                ...
                for reaction in reactions {
                    ...
                    let revision_call = if workflow_revisions.is_empty() {
                        ...
                        format!(
                            "workflowRevisions, err := workflow{command}Revisions(input, outcome); if err != nil {{ return CommandResult[{command}Outcome]{{}}, err }}; "
                        )
                    };
                    let revision_field = if workflow_revisions.is_empty() {
                        String::new()
