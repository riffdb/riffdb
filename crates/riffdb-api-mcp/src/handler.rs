...
pub enum McpToolDiscoveryItem {
    /// One exact accepted fixed-tool tag in `1..=14`.
    Fixed(u8),
    ...
    for item in items {
        match item {
            McpToolDiscoveryItem::Fixed(tag) if dynamic.is_none() => {
                if !(1..=14).contains(tag) || fixed.is_some_and(|prior| prior >= *tag) {
                    return false;
                }
                fixed = Some(*tag);
            }
...
mod tests {
    ...
    fn tools_list_schema_ownership_is_registry_driven_and_catalog_is_unconditional() {
        ...
        let fixed_items: Vec<_> = (1..=14).map(McpToolDiscoveryItem::Fixed).collect();
