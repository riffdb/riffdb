...
const STORAGE_SOURCES: &[&str] = &[
    ...
    "riffdb/storage/v1/command_prefix_authority_v7.proto",
    ...
    "riffdb/storage/v1/installation_v1.proto",
...
const DURABLE_RECORDS: &[DurableRecord] = &[
    ...
    durable(
        "validated_prefix_checkpoint_v1.proto",
    ...
    durable(
        "capability_installation.proto",
        "CapabilityRecordV3",
        PayloadBound::Document,
    ),
    durable(
        "capability_row_policy.proto",
        "CapabilityRecordV4",
        PayloadBound::Document,
    ),
    durable(
        "event_policy_anchor.proto",
        "StoredDurableEventV2",
        PayloadBound::Document,
    ),
    durable(
        "capability_export.proto",
        "CapabilityRecordV5",
        PayloadBound::Document,
    ),
    durable(
    ...
    durable(
        "vector_health_observation_v1.proto",
    ...
    durable(
        "command_prefix_authority_v7.proto",
...
const EXPECTED_METHODS: &[(&str, &str, bool)] = &[
    ...
    ("ApplicationQueryService", "InspectVectorState", false),
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/descriptors/riffdb-v1-descriptor-set.bin",
        &production.encode_to_vec(),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/descriptors/riffdb-storage-v1-descriptor-set.bin",
        &storage.encode_to_vec(),
    )?;
    write_artifact(
        &output_root,
    ...
    let public_schema_hashes = public_schema_hashes(&production);
    write_artifact(
        &output_root,
        "fixtures/proto/public-schema-hashes.txt",
        public_schema_hashes.as_bytes(),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/public-schema-hashes.txt",
        public_schema_hashes.as_bytes(),
    )?;
    write_artifact(
        &output_root,
    ...
    let validated_prefix_checkpoint_record = durable_registry
        .get(current_v1_record_count + 14)
        .ok_or_else(|| {
            io::Error::other("durable validated-prefix-checkpoint registry is incomplete")
        })?;
    let retention_watermark_record = durable_registry
        .get(current_v1_record_count + 15)
        .ok_or_else(|| io::Error::other("durable retention-watermark registry is incomplete"))?;
    let retention_holds_record = durable_registry
        .get(current_v1_record_count + 16)
        .ok_or_else(|| io::Error::other("durable retention-holds registry is incomplete"))?;
    ...
    let capability_v3_record = durable_registry
        .get(current_v1_record_count + 45)
        .ok_or_else(|| io::Error::other("durable capability-v3 registry is incomplete"))?;
    let capability_v4_record = durable_registry
        .get(current_v1_record_count + 46)
        .ok_or_else(|| io::Error::other("durable capability-v4 registry is incomplete"))?;
    let durable_event_v2_record = durable_registry
        .get(current_v1_record_count + 47)
        .ok_or_else(|| io::Error::other("durable event-v2 registry is incomplete"))?;
    ...
    let vector_health_observation_record = durable_registry
    ...
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-readable-registry.txt",
        durable_readable_registry_fixture(&durable_registry)?.as_bytes(),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-writable-registry.txt",
        durable_writable_registry_fixture(&durable_registry)?.as_bytes(),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        ...
        &durable_schema_hashes(event_reference_v2_records),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        ...
        &durable_schema_hashes(migration_v1_records),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        ...
        &durable_record_bounds(std::slice::from_ref(validated_prefix_checkpoint_record)),
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-retention-watermark-v1-schema-hash.bin",
        &retention_watermark_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-retention-watermark-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(retention_watermark_record)),
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-retention-holds-v1-schema-hash.bin",
        &retention_holds_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-retention-holds-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(retention_holds_record)),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        ...
        &installation_v1_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        ...
        &capability_v7_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-evidence-v1-schema-hash.bin",
        &vector_evidence_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-evidence-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_evidence_record)),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-evidence-v1-schema-hash.bin",
        &vector_evidence_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-evidence-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_evidence_record)),
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-observation-v1-schema-hash.bin",
        &vector_observation_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-observation-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_observation_record)),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-observation-v1-schema-hash.bin",
        &vector_observation_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-observation-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_observation_record)),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-health-observation-v1-schema-hash.bin",
        &vector_health_observation_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-vector-health-observation-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_health_observation_record)),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-health-observation-v1-schema-hash.bin",
        &vector_health_observation_record.schema_hash,
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-vector-health-observation-v1-record-bound.bin",
        &durable_record_bounds(std::slice::from_ref(vector_health_observation_record)),
    )?;
    write_artifact(
        &output_root,
    ...
    write_artifact(
        &output_root,
        "fixtures/proto/durable-command-prefix-authority-v7-schema-hashes.bin",
        &durable_schema_hashes(command_prefix_records),
    )?;
    write_artifact(
        &output_root,
        "fixtures/proto/durable-command-prefix-authority-v7-record-bounds.bin",
        &durable_record_bounds(command_prefix_records),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-command-prefix-authority-v7-schema-hashes.bin",
        &durable_schema_hashes(command_prefix_records),
    )?;
    write_artifact(
        &output_root,
        "crates/riffdb-proto/fixtures/durable-command-prefix-authority-v7-record-bounds.bin",
        &durable_record_bounds(command_prefix_records),
    )?;
    write_artifact(
        &output_root,
    ...
    for (stem, record) in [
        ...
        ("service-audit-v2", service_audit_v2_record),
        ("history-tombstone-v1", history_tombstone_record),
        (
            "retention-administration-v1",
...
fn durable_writable_registry_fixture(
    ...
    let retention_watermark = records.get(current_v1_record_count + 15).ok_or_else(|| {
        io::Error::other("durable registry is missing StoredRetentionWatermarkV1")
    })?;
    let retention_holds = records
        .get(current_v1_record_count + 16)
        .ok_or_else(|| io::Error::other("durable registry is missing StoredRetentionHoldsV1"))?;
    ...
    let capability_v3 = records
        .get(current_v1_record_count + 45)
        .ok_or_else(|| io::Error::other("durable registry is missing CapabilityRecordV3"))?;
    let capability_v4 = records
        .get(current_v1_record_count + 46)
        .ok_or_else(|| io::Error::other("durable registry is missing CapabilityRecordV4"))?;
    let durable_event_v2 = records
        .get(current_v1_record_count + 47)
        .ok_or_else(|| io::Error::other("durable registry is missing StoredDurableEventV2"))?;
    ...
    let writable = legacy[..8]
        ...
        .chain(std::iter::once(durable_event_v2))
