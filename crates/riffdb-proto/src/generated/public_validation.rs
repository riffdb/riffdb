...
const MAX_AUTHENTICATED_HEALTH_COMPONENTS: usize = 7;
        ...
        .ok_or(PublicWireError::MissingRequiredField)?
    {
        ...
        v1::health_response::Result::Authenticated(health) => {
            ...
            for component in &health.components {
                ...
                match (kind, &component.replication) {
                    (v1::HealthComponentKind::Replication, Some(progress)) => {
                        validate_replication_statistics(progress)?;
                        let healthy = progress.registered_followers == Some(0)
                            || (progress.application_lag_sequences == Some(0)
                    ...
                    (v1::HealthComponentKind::Replication, None) | (_, None) => {},
                    (_, Some(_)) => return Err(PublicWireError::NonCanonical),
                }
...
fn replication_frontier_pair(value: &v1::ReplicationFrontier) -> Result<(u64, u64), PublicWireError> {
    fn position(value: Option<&v1::FrontierPosition>) -> Result<u64, PublicWireError> {
        validate_frontier(value)?;
        match value.and_then(|v| v.position.as_ref()) {
            Some(v1::frontier_position::Position::BeforeFirst(_)) => Ok(0),
            Some(v1::frontier_position::Position::AppliedThrough(v)) => Ok(*v),
            _ => Err(PublicWireError::MissingRequiredField),
        }
    }
    Ok((position(value.application.as_ref())?, position(value.administration.as_ref())?))
}
fn validate_replication_statistics(value: &v1::ReplicationStatistics) -> Result<(), PublicWireError> {
    let applied = replication_frontier_pair(value.applied_frontier.as_ref().ok_or(PublicWireError::MissingRequiredField)?)?;
    let source = value.source_frontier.as_ref().map(replication_frontier_pair).transpose()?;
    let ack = value.acknowledged_frontier.as_ref().map(replication_frontier_pair).transpose()?;
    if ack.is_some_and(|ack| ack.0 > applied.0 || ack.1 > applied.1) {
        return Err(PublicWireError::NonCanonical);
    }
    let target = match v1::ReplicationRole::try_from(value.role) {
        Ok(v1::ReplicationRole::Primary) => {
            let count = value.registered_followers.ok_or(PublicWireError::MissingRequiredField)?;
            if source != Some(applied) || (count == 0) != ack.is_none() {
                return Err(PublicWireError::NonCanonical);
            }
            ack
        }
        Ok(v1::ReplicationRole::Follower) => {
            if value.registered_followers.is_some() { return Err(PublicWireError::NonCanonical); }
            Some(applied)
        }
        _ => return Err(PublicWireError::InvalidEnum),
    };
    let expected = source.zip(target).map(|(head, target)| {
        Ok::<_, PublicWireError>((head.0.checked_sub(target.0).ok_or(PublicWireError::NonCanonical)?,
            head.1.checked_sub(target.1).ok_or(PublicWireError::NonCanonical)?))
    }).transpose()?;
    if value.application_lag_sequences != expected.map(|lag| lag.0)
        || value.administration_lag_sequences != expected.map(|lag| lag.1) {
        return Err(PublicWireError::NonCanonical);
    }
    Ok(())
}
...
fn validate_stats_response(message: &v1::StatsResponse) -> Result<(), PublicWireError> {
    ...
    if let Some(replication) = &message.replication { validate_replication_statistics(replication)?; }
    Ok(())
}
...
fn permission_key(
    ...
    let key: (u8, &str, u32, &[u8], &str) = match permission {
        ...
        v1::capability_permission::Permission::FenceReplicationPrimary(_) => (34, "", 0, &[], ""),
    };
    if key.0 >= 3
...
fn validate_capability_grant(grant: Option<&v1::CapabilityGrant>) -> Result<(), PublicWireError> {
    ...
    let replication = grant.permissions.iter().any(|permission| {
        matches!(permission.permission.as_ref(),
            Some(v1::capability_permission::Permission::ReplicateChangelog(_)
                        | v1::capability_permission::Permission::FenceReplicationPrimary(_)))
    });
    if replication {
    ...
    for approval in &grant.approval_required {
        if (!(1..=24).contains(approval) && *approval != 34) || *approval <= previous_approval {
            return Err(PublicWireError::NonCanonical);
        }
...
fn preflight_health_component(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 3, &[], &[], &[
        NestedRule { field: 3, preflight: preflight_replication_statistics },
    ], &[])
}
fn preflight_replication_frontier(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 2, &[], &[], &[
        NestedRule { field: 1, preflight: preflight_frontier },
        NestedRule { field: 2, preflight: preflight_frontier },
    ], &[])
}
fn preflight_replication_statistics(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_root(input, 512, 7, &[], &[])?;
    preflight_nested_message(input, 7, &[], &[], &[
        NestedRule { field: 2, preflight: preflight_replication_frontier },
        NestedRule { field: 3, preflight: preflight_replication_frontier },
        NestedRule { field: 4, preflight: preflight_replication_frontier },
    ], &[])
}
fn preflight_stats_response(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 7, &[], &[], &[
        NestedRule { field: 7, preflight: preflight_replication_statistics },
    ], &[])
}
...
const MAX_REPLICATION_FENCE_WIRE_BYTES: usize = 2048;
...
fn preflight_replication_fence_request(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 4, &[], &[], &[
        NestedRule { field: 4, preflight: preflight_replication_position },
    ], &[])
}
fn preflight_replication_fence_evidence(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_root(input, MAX_REPLICATION_FENCE_WIRE_BYTES, 5, &[], &[])?;
    preflight_nested_message(input, 5, &[], &[], &[
        NestedRule { field: 2, preflight: preflight_replication_position },
        NestedRule { field: 3, preflight: preflight_replication_position },
        NestedRule { field: 4, preflight: preflight_replication_position },
        NestedRule { field: 5, preflight: preflight_replication_position },
    ], &[])
}
fn preflight_replication_request(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 13, &[], &[&[7, 10, 11, 13]], &[
        ...
        NestedRule { field: 13, preflight: preflight_replication_fence_request },
    ], &[])
}

fn preflight_replication_source_head(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_root(input, 128, 3, &[], &[])?;
    preflight_nested_message(input, 3, &[], &[], &[
        NestedRule { field: 2, preflight: preflight_frontier },
        NestedRule { field: 3, preflight: preflight_frontier },
    ], &[])
...
fn preflight_replication_response(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_root(input, MAX_REPLICATION_FRAME_WIRE_BYTES + 1024, 6, &[], &[&[1, 2, 3, 4, 6]])?;
    let mut cursor = Cursor::new(input);
    while let Some(field) = cursor.next().map_err(|_| PublicWireError::MalformedEncoding)? {
        if field.number == 5 {
            preflight_replication_source_head(wire_field_bytes(field)?)?;
        } else if field.number == 6 {
            if input.len() > MAX_REPLICATION_FENCE_WIRE_BYTES {
                return Err(PublicWireError::MessageTooLarge);
            }
            preflight_replication_fence_evidence(wire_field_bytes(field)?)?;
        }
    }
    Ok(())
}
...
fn validate_stream_changelog_request(message: &v1::StreamChangelogRequest) -> Result<(), PublicWireError> {
    ...
    match (&message.after, &message.bootstrap, &message.attachment, &message.fence_evidence) {
        (Some(after), None, None, None) => validate_replication_position(after)?,
        ...
        (None, None, None, Some(fence)) => {
            if !valid_uuid(&fence.operation_id, riffdb_types::ReplicationFenceOperationId::from_bytes)
                || fence.hold_id.len() != 16 || fence.hold_id.iter().all(|byte| *byte == 0)
                || fence.registration_generation == 0 {
                return Err(PublicWireError::InvalidIdentity);
            }
            validate_replication_position(fence.applied.as_ref().ok_or(PublicWireError::MissingRequiredField)?)?;
        }
...
fn validate_stream_changelog_response(message: &v1::StreamChangelogResponse) -> Result<(), PublicWireError> {
    if let Some(head) = &message.source_head {
        if !matches!(message.item, Some(v1::stream_changelog_response::Item::Frame(_)))
            || head.transaction_sequence == 0 {
            return Err(PublicWireError::InvalidIdentity);
        }
        validate_frontier(head.application_frontier.as_ref())?;
        validate_frontier(head.administration_frontier.as_ref())?;
    }
    match message.item.as_ref().ok_or(PublicWireError::MissingRequiredField)? {
        ...
        v1::stream_changelog_response::Item::FenceEvidence(evidence) => {
            if prost::Message::encoded_len(message) > MAX_REPLICATION_FENCE_WIRE_BYTES {
                return Err(PublicWireError::MessageTooLarge);
            }
            validate_replication_fence_evidence(evidence)
        },
...
fn validate_replication_fence_evidence(evidence: &v1::ReplicationFenceEvidence) -> Result<(), PublicWireError> {
    if prost::Message::encoded_len(evidence) > MAX_REPLICATION_FENCE_WIRE_BYTES
        || evidence.fence_record.is_empty() || evidence.fence_record.len() > 1536 {
        return Err(PublicWireError::InvalidIdentity);
    }
    for point in [&evidence.applied, &evidence.anchor, &evidence.tail, &evidence.minimum_resume] {
        validate_replication_position(point.as_ref().ok_or(PublicWireError::MissingRequiredField)?)?;
    }
    Ok(())
}

impl_public_message!(v1::StreamChangelogRequest, 1024, 13, &[], &[&[7, 10, 11, 13]], preflight_replication_request, validate_stream_changelog_request);
impl_public_message!(v1::StreamChangelogResponse, MAX_REPLICATION_FRAME_WIRE_BYTES + 1024, 6, &[], &[&[1, 2, 3, 4, 6]], preflight_replication_response, validate_stream_changelog_response);
...
fn preflight_primary_fence_request(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 4, &[], &[], &[NestedRule { field: 3, preflight: preflight_follower_target }], &[])
}
fn preflight_primary_fence_receipt(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 6, &[], &[], &[
        NestedRule { field: 2, preflight: preflight_follower_target },
        NestedRule { field: 5, preflight: preflight_frontier }], &[])
}
fn preflight_primary_fence_response(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 2, &[], &[&[1, 2]], &[NestedRule { field: 1, preflight: preflight_primary_fence_receipt }], &[])
}
fn validate_primary_fence_request(value: &v1::FenceReplicationPrimaryRequest) -> Result<(), PublicWireError> {
    request_id(&value.request_id)?;
    validate_follower_target(value.target.as_ref())?;
    if !valid_uuid(&value.operation_id, riffdb_types::ReplicationFenceOperationId::from_bytes)
        || value.registration_generation == 0 { return Err(PublicWireError::InvalidIdentity); }
    Ok(())
}
fn validate_primary_fence_response(value: &v1::FenceReplicationPrimaryResponse) -> Result<(), PublicWireError> {
    match value.result.as_ref().ok_or(PublicWireError::MissingRequiredField)? {
        v1::fence_replication_primary_response::Result::Receipt(receipt) => {
            if !valid_uuid(&receipt.operation_id, riffdb_types::ReplicationFenceOperationId::from_bytes)
                || receipt.registration_generation == 0 || receipt.administration_sequence == 0 {
                return Err(PublicWireError::InvalidIdentity);
            }
            validate_follower_target(receipt.target.as_ref())?;
            validate_frontier(receipt.final_application_frontier.as_ref())
        }
        v1::fence_replication_primary_response::Result::Refusal(1..=3) => Ok(()),
        v1::fence_replication_primary_response::Result::Refusal(_) => Err(PublicWireError::InvalidEnum),
    }
}
/// A fence response must retain the exact selected operation, follower and generation.
/// This checks a summary only; serialized receipts never authenticate a source.
pub fn validate_fence_replication_primary_exchange(request: &v1::FenceReplicationPrimaryRequest,
    response: &v1::FenceReplicationPrimaryResponse) -> Result<(), PublicWireError> {
    validate_public_message(request)?;
    validate_public_message(response)?;
    if let Some(v1::fence_replication_primary_response::Result::Receipt(receipt)) = &response.result
        && (receipt.operation_id != request.operation_id || receipt.target != request.target
            || receipt.registration_generation != request.registration_generation) {
        return Err(PublicWireError::InconsistentFields);
    }
    Ok(())
}
impl_public_message!(v1::FenceReplicationPrimaryRequest, MAX_PUBLIC_REQUEST_BYTES, 4, &[], &[], preflight_primary_fence_request, validate_primary_fence_request);
impl_public_message!(v1::FenceReplicationPrimaryResponse, MAX_PUBLIC_RESPONSE_BYTES, 2, &[], &[&[1, 2]], preflight_primary_fence_response, validate_primary_fence_response);

fn preflight_promote_follower_request(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 5, &[], &[], &[NestedRule { field: 4, preflight: preflight_follower_target }], &[])
}
fn preflight_promotion_receipt(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 9, &[], &[], &[
        NestedRule { field: 2, preflight: preflight_follower_target },
        NestedRule { field: 5, preflight: preflight_frontier }], &[])
}
fn preflight_promote_follower_response(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_nested_message(input, 1, &[], &[], &[NestedRule { field: 1, preflight: preflight_promotion_receipt }], &[])
}
fn validate_promote_follower_request(value: &v1::PromoteFollowerRequest) -> Result<(), PublicWireError> {
    request_id(&value.request_id)?;
    validate_follower_target(value.target.as_ref())?;
    if !valid_uuid(&value.operation_id, riffdb_types::ReplicationPromotionOperationId::from_bytes)
        || !valid_uuid(&value.fence_operation_id, riffdb_types::ReplicationFenceOperationId::from_bytes)
        || value.registration_generation == 0 { return Err(PublicWireError::InvalidIdentity); }
    Ok(())
}
fn validate_promote_follower_response(value: &v1::PromoteFollowerResponse) -> Result<(), PublicWireError> {
    let receipt = value.receipt.as_ref().ok_or(PublicWireError::MissingRequiredField)?;
    validate_follower_target(receipt.target.as_ref())?;
    validate_frontier(receipt.applied_application_frontier.as_ref())?;
    let target = receipt.target.as_ref().ok_or(PublicWireError::MissingRequiredField)?;
    if !valid_uuid(&receipt.operation_id, riffdb_types::ReplicationPromotionOperationId::from_bytes)
        || receipt.registration_generation == 0 || receipt.administration_sequence == 0 {
        return Err(PublicWireError::InvalidIdentity);
    }
    if target.history_incarnation.checked_add(1) != Some(receipt.history_incarnation)
        || target.leadership_epoch.checked_add(1) != Some(receipt.leadership_epoch) {
        return Err(PublicWireError::InconsistentFields);
    }
    let applied = match receipt.applied_application_frontier.as_ref().and_then(|f| f.position.as_ref()) {
        Some(v1::frontier_position::Position::AppliedThrough(n)) => *n,
        Some(v1::frontier_position::Position::BeforeFirst(_)) => 0,
        None => return Err(PublicWireError::MissingRequiredField),
    };
    applied.checked_add(receipt.application_rpo).ok_or(PublicWireError::InconsistentFields)?;
    Ok(())
}
/// Correlates a promotion summary to the exact request; it grants no fence authority.
pub fn validate_promote_follower_exchange(request: &v1::PromoteFollowerRequest,
    response: &v1::PromoteFollowerResponse) -> Result<(), PublicWireError> {
    validate_public_message(request)?;
    validate_public_message(response)?;
    let receipt = response.receipt.as_ref().ok_or(PublicWireError::MissingRequiredField)?;
    if receipt.operation_id != request.operation_id || receipt.target != request.target
        || receipt.registration_generation != request.registration_generation {
        return Err(PublicWireError::InconsistentFields);
    }
    Ok(())
}
impl_public_message!(v1::PromoteFollowerRequest, MAX_PUBLIC_REQUEST_BYTES, 5, &[], &[], preflight_promote_follower_request, validate_promote_follower_request);
impl_public_message!(v1::PromoteFollowerResponse, MAX_PUBLIC_RESPONSE_BYTES, 1, &[], &[], preflight_promote_follower_response, validate_promote_follower_response);
...
pub const GENERATED_PUBLIC_OPERATION_COUNT: usize = 62;
/// Descriptor-checked registry row used by generated validation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct GeneratedPublicValidationMessage {
    /// Stable semantic operation tag.
    pub tag: u8,
    /// Semantic operation variant name.
    pub operation: &'static str,
    /// Fully qualified Protobuf request message.
    pub request: &'static str,
    /// Descriptor-declared request field count.
    pub request_fields: usize,
    /// Fully qualified Protobuf response or stream-item message.
    pub response: &'static str,
    /// Descriptor-declared response field count.
    pub response_fields: usize,
    /// Inclusive encoded request-byte maximum.
    pub request_bytes: usize,
    /// Inclusive encoded response-byte maximum.
    pub response_bytes: usize,
}
/// Descriptor-checked request and response identities in semantic-tag order.
pub const GENERATED_PUBLIC_VALIDATION_MESSAGES: &[GeneratedPublicValidationMessage] = &[
    GeneratedPublicValidationMessage { tag: 1, operation: "ValidateContract", request: "riffdb.v1.ValidateContractRequest", request_fields: 3, response: "riffdb.v1.ValidateContractResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 2, operation: "ExplainCommand", request: "riffdb.v1.ExplainCommandRequest", request_fields: 3, response: "riffdb.v1.ExplainCommandResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 3, operation: "DeployContract", request: "riffdb.v1.DeployContractRequest", request_fields: 5, response: "riffdb.v1.DeployContractResponse", response_fields: 8, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 4, operation: "GetActiveContract", request: "riffdb.v1.GetActiveContractRequest", request_fields: 1, response: "riffdb.v1.GetActiveContractResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 5, operation: "GetContractVersion", request: "riffdb.v1.GetContractVersionRequest", request_fields: 3, response: "riffdb.v1.GetContractVersionResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 6, operation: "ExecuteCommand", request: "riffdb.v1.ExecuteCommandRequest", request_fields: 4, response: "riffdb.v1.ExecuteCommandResponse", response_fields: 10, request_bytes: 8388608, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 7, operation: "ResolveCommandOutcome", request: "riffdb.v1.GetOutcomeRequest", request_fields: 5, response: "riffdb.v1.GetOutcomeResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 8, operation: "GetEntity", request: "riffdb.v1.GetEntityRequest", request_fields: 5, response: "riffdb.v1.GetEntityResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 9, operation: "ScanIndex", request: "riffdb.v1.ScanIndexRequest", request_fields: 6, response: "riffdb.v1.ScanIndexResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 10, operation: "QueryProjection", request: "riffdb.v1.QueryProjectionRequest", request_fields: 7, response: "riffdb.v1.QueryProjectionResponse", response_fields: 4, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 11, operation: "GetProjectionStatus", request: "riffdb.v1.GetProjectionStatusRequest", request_fields: 3, response: "riffdb.v1.GetProjectionStatusResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 12, operation: "GetCommit", request: "riffdb.v1.GetCommitRequest", request_fields: 3, response: "riffdb.v1.GetCommitResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 13, operation: "ScanCommits", request: "riffdb.v1.ScanCommitsRequest", request_fields: 3, response: "riffdb.v1.ScanCommitsResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 14, operation: "SubscribeToCommits", request: "riffdb.v1.SubscribeCommitsRequest", request_fields: 4, response: "riffdb.v1.CommitNotification", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 15, operation: "TraceProvenance", request: "riffdb.v1.TraceProvenanceRequest", request_fields: 2, response: "riffdb.v1.TraceProvenanceResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 16, operation: "GetHealth", request: "riffdb.v1.HealthRequest", request_fields: 1, response: "riffdb.v1.HealthResponse", response_fields: 4, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 17, operation: "GetStatistics", request: "riffdb.v1.StatsRequest", request_fields: 1, response: "riffdb.v1.StatsResponse", response_fields: 7, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 18, operation: "CreateCapability", request: "riffdb.v1.CreateCapabilityRequest", request_fields: 8, response: "riffdb.v1.CreateCapabilityResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 19, operation: "RevokeCapability", request: "riffdb.v1.RevokeCapabilityRequest", request_fields: 3, response: "riffdb.v1.RevokeCapabilityResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 20, operation: "ListPendingOutboxDeliveries", request: "riffdb.v1.ListPendingOutboxDeliveriesRequest", request_fields: 2, response: "riffdb.v1.ListPendingOutboxDeliveriesResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 21, operation: "DiscoverCommandTools", request: "riffdb.v1.DiscoverCommandToolsRequest", request_fields: 4, response: "riffdb.v1.DiscoverCommandToolsResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 22, operation: "DiscoverResources", request: "riffdb.v1.DiscoverResourcesRequest", request_fields: 5, response: "riffdb.v1.DiscoverResourcesResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 23, operation: "DescribeContract", request: "riffdb.app.v1.DescribeContractRequest", request_fields: 2, response: "riffdb.app.v1.DescribeContractResponse", response_fields: 4, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 24, operation: "CheckQuery", request: "riffdb.app.v1.CheckQueryRequest", request_fields: 3, response: "riffdb.app.v1.CheckQueryResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 25, operation: "ExplainQuery", request: "riffdb.app.v1.ExplainQueryRequest", request_fields: 5, response: "riffdb.app.v1.ExplainQueryResponse", response_fields: 4, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 26, operation: "ExecuteQuery", request: "riffdb.app.v1.ExecuteQueryRequest", request_fields: 10, response: "riffdb.app.v1.ExecuteQueryResponse", response_fields: 8, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 27, operation: "DeployQueryModule", request: "riffdb.app.v1.DeployQueryModuleRequest", request_fields: 8, response: "riffdb.app.v1.DeployQueryModuleResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 28, operation: "ApplyContractMigration", request: "riffdb.v1.ApplyContractMigrationRequest", request_fields: 6, response: "riffdb.v1.ApplyContractMigrationResponse", response_fields: 2, request_bytes: 33554432, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 29, operation: "DescribeEvent", request: "riffdb.v1.DescribeEventRequest", request_fields: 2, response: "riffdb.v1.DescribeEventResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 30, operation: "ReplayEvents", request: "riffdb.v1.ReplayEventsRequest", request_fields: 5, response: "riffdb.v1.ReplayEventsResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 31, operation: "TailEvents", request: "riffdb.v1.TailEventsRequest", request_fields: 6, response: "riffdb.v1.TailEventsResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 32, operation: "ExecuteProjectedQuery", request: "riffdb.app.v1.ExecuteProjectedQueryRequest", request_fields: 6, response: "riffdb.app.v1.ExecuteProjectedQueryResponse", response_fields: 8, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 33, operation: "DeployReactiveModule", request: "riffdb.app.v1.DeployReactiveModuleRequest", request_fields: 4, response: "riffdb.app.v1.DeployReactiveModuleResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 34, operation: "ConsumeEventStream", request: "riffdb.v1.ConsumeEventStreamRequest", request_fields: 7, response: "riffdb.v1.ConsumeEventStreamResponse", response_fields: 5, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 35, operation: "AcknowledgeEventStream", request: "riffdb.v1.AcknowledgeEventStreamRequest", request_fields: 5, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 36, operation: "NegativeAcknowledgeEventStream", request: "riffdb.v1.NegativeAcknowledgeEventStreamRequest", request_fields: 6, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 37, operation: "SeekEventStreamConsumer", request: "riffdb.v1.SeekEventStreamConsumerRequest", request_fields: 4, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 38, operation: "RetireEventStreamConsumer", request: "riffdb.v1.RetireEventStreamConsumerRequest", request_fields: 2, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 39, operation: "GetEventStreamConsumerStatus", request: "riffdb.v1.GetEventStreamConsumerStatusRequest", request_fields: 2, response: "riffdb.v1.GetEventStreamConsumerStatusResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 40, operation: "WatchNamedQuery", request: "riffdb.v1.WatchNamedQueryRequest", request_fields: 5, response: "riffdb.v1.LiveQueryUpdate", response_fields: 5, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 41, operation: "ConsumeContextualSubscription", request: "riffdb.v1.ConsumeContextualSubscriptionRequest", request_fields: 4, response: "riffdb.v1.ConsumeContextualSubscriptionResponse", response_fields: 5, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 42, operation: "AcknowledgeContextualSubscription", request: "riffdb.v1.AcknowledgeContextualSubscriptionRequest", request_fields: 5, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 43, operation: "NegativeAcknowledgeContextualSubscription", request: "riffdb.v1.NegativeAcknowledgeContextualSubscriptionRequest", request_fields: 6, response: "riffdb.v1.EventConsumerMutationResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 44, operation: "GetContextualSubscriptionStatus", request: "riffdb.v1.GetContextualSubscriptionStatusRequest", request_fields: 2, response: "riffdb.v1.GetEventStreamConsumerStatusResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 45, operation: "ExecuteContextualReaction", request: "riffdb.v1.ExecuteContextualReactionRequest", request_fields: 5, response: "riffdb.v1.ExecuteCommandResponse", response_fields: 10, request_bytes: 8388608, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 46, operation: "GetReactiveWakeup", request: "riffdb.v1.GetReactiveWakeupRequest", request_fields: 1, response: "riffdb.v1.GetReactiveWakeupResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 47, operation: "StartApplicationInstallation", request: "riffdb.v1.StartApplicationInstallationRequest", request_fields: 4, response: "riffdb.v1.StartApplicationInstallationResponse", response_fields: 2, request_bytes: 4325376, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 48, operation: "GetApplicationInstallation", request: "riffdb.v1.GetApplicationInstallationRequest", request_fields: 2, response: "riffdb.v1.GetApplicationInstallationResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 49, operation: "StartApplicationExport", request: "riffdb.v1.StartApplicationExportRequest", request_fields: 5, response: "riffdb.v1.StartApplicationExportResponse", response_fields: 3, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 50, operation: "GetApplicationExportPage", request: "riffdb.v1.GetApplicationExportPageRequest", request_fields: 4, response: "riffdb.v1.GetApplicationExportPageResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 51, operation: "GetApplicationExport", request: "riffdb.v1.GetApplicationExportRequest", request_fields: 2, response: "riffdb.v1.GetApplicationExportResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 52, operation: "CancelApplicationExport", request: "riffdb.v1.CancelApplicationExportRequest", request_fields: 2, response: "riffdb.v1.CancelApplicationExportResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 53, operation: "StartApplicationReimport", request: "riffdb.v1.StartApplicationReimportRequest", request_fields: 7, response: "riffdb.v1.StartApplicationReimportResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 54, operation: "ApplyApplicationReimportPage", request: "riffdb.v1.ApplyApplicationReimportPageRequest", request_fields: 3, response: "riffdb.v1.ApplyApplicationReimportPageResponse", response_fields: 1, request_bytes: 4325376, response_bytes: 4194304 },
    ...
    GeneratedPublicValidationMessage { tag: 60, operation: "FenceReplicationPrimary", request: "riffdb.v1.FenceReplicationPrimaryRequest", request_fields: 4, response: "riffdb.v1.FenceReplicationPrimaryResponse", response_fields: 2, request_bytes: 1048576, response_bytes: 4194304 },
    GeneratedPublicValidationMessage { tag: 61, operation: "StreamChangelog", request: "riffdb.v1.StreamChangelogRequest", request_fields: 13, response: "riffdb.v1.StreamChangelogResponse", response_fields: 6, request_bytes: 1024, response_bytes: 33555456 },
    GeneratedPublicValidationMessage { tag: 62, operation: "PromoteFollower", request: "riffdb.v1.PromoteFollowerRequest", request_fields: 5, response: "riffdb.v1.PromoteFollowerResponse", response_fields: 1, request_bytes: 1048576, response_bytes: 4194304 },
];
