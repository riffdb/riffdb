...
fn preflight_replication_response(input: &[u8]) -> Result<(), PublicWireError> {
    preflight_root(input, MAX_REPLICATION_FRAME_WIRE_BYTES + 1024, 5, &[], &[&[1, 2, 3, 4]])?;
    let mut cursor = Cursor::new(input);
...
impl_public_message!(v1::StreamChangelogResponse, MAX_REPLICATION_FRAME_WIRE_BYTES + 1024, 5, &[], &[&[1, 2, 3, 4]], preflight_replication_response, validate_stream_changelog_response);
