...
fn encode_schema(writer: &mut Writer, schema: &SchemaIr) -> Result<(), IrValidationError> {
    ...
    if !schema.vector_ann_specs().is_empty() {
        writer.u32(VECTOR_ANN_SPEC_SCHEMA_EXTENSION)?;
        writer.u32(schema.vector_ann_specs().len() as u32)?;
        for spec in schema.vector_ann_specs() {
            writer.u32(spec.entity().get())?;
            writer.u32(spec.field().get())?;
            writer.u32(spec.row_threshold())?;
...
fn encode_binding(
    ...
    if ir_version >= EXECUTABLE_IR_VERSION_V17 {
        writer.u32(binding.initializer().len() as u32)?;
        for field in binding.initializer() {
            writer.u32(field.field_id().get())?;
            writer.u32(field.expression().get())?;
        }
        writer.bool(binding.failure().is_some())?;
        ...
        let failure = binding
            .failure()
            .ok_or(IrValidationError::UnsupportedVersion {
                kind: "initialized mutable binding executable IR",
                value: ir_version,
            })?;
        encode_outcome_construction(writer, failure)?;
    }
...
fn decode_schema(reader: &mut Reader<'_>) -> Result<SchemaIr, IrValidationError> {
    ...
    let mut vector_ann_specs = Vec::new();
    if reader.remaining() >= 4 && reader.peek_u32()? == VECTOR_ANN_SPEC_SCHEMA_EXTENSION {
        let _marker = reader.u32()?;
        let spec_count = decode_len(reader, "vector ANN specs", crate::MAX_DECLARATIONS_PER_KIND)?;
        vector_ann_specs.reserve(spec_count);
        for _ in 0..spec_count {
            vector_ann_specs.push(crate::VectorAnnSpecV1::new(
                decode_entity_id(reader)?,
...
fn decode_binding(
    ...
    let initializer = if ir_version >= EXECUTABLE_IR_VERSION_V17 {
        let count = decode_len(
            reader,
            "binding initializer fields",
            crate::MAX_COMMAND_ITEMS,
        )?;
        let mut fields = Vec::with_capacity(count);
        for _ in 0..count {
    ...
    let failure = if ir_version >= EXECUTABLE_IR_VERSION_V17 {
        if reader.bool()? {
    ...
    if matches!(
        ...
        BindingPlan::new_with_delete_failures(
            ...
            accessed_fields,
            complete_record_access,
            failure.ok_or(IrValidationError::InvalidDependency {
                reason: "ordinary binding omits its failure outcome",
            })?,
            restriction_failure,
            cascade_failure,
        )
