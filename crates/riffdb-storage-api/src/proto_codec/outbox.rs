...
use super::{
    ...
    event_id_from_proto, event_id_to_proto, event_to_proto, require, timestamp_from_proto,
    timestamp_to_proto,
};

const INTENT: &str = "riffdb.storage.v1.StoredOutboxIntentV1";
const STATUS: &str = "riffdb.storage.v1.StoredOutboxStatusV1";

/// Encodes one authoritative outbox intent containing the reciprocal event.
pub fn encode_outbox_intent_v1(
    ...
    encode_message(
        ...
        &wire::StoredOutboxIntentV1 {
            event: Some(event_to_proto(value.event())),
        },
    )
}

/// Decodes one authoritative outbox intent and verifies its nested event hash.
pub fn decode_outbox_intent_v1(
    encoded: &[u8],
) -> Result<EncodedPageItem<StoredOutboxIntentV1>, DurableCodecError> {
    decode_message::<wire::StoredOutboxIntentV1, _, _>(INTENT, encoded, |value| {
