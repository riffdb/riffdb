...
fn lexes_every_reserved_keyword() {
    let source = concat!(
        "contract version entity key field invariant index event enum aggregate root child ",
        "partition_by conflict_key projection source where measure count sum frontier ",
        "transactionally_ordered command input idempotency_key read mutate create as else ",
