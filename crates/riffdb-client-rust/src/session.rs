...
impl ApplicationSessionIdentity {
    ...
    fn framed_open_request(
        &self,
        request_id: Vec<u8>,
        credential: &[u8],
        database: &riffdb_types::DatabaseAlias,
    ) -> v1::ApplicationSessionOpen {
        let mut request = self.open_request(request_id);
        request.credential_presentation = credential.to_vec();
        request.database_alias = database.as_str().to_owned();
        request
    }
...
impl BoundedApplicationSession {
    ...
    pub(crate) async fn open_framed_loopback(
        address: SocketAddr,
        identity: ApplicationSessionIdentity,
        metadata: CallMetadata,
    ) -> Result<Self, ClientError> {
        ...
        let (credential, database) = metadata
            ...
            .ok_or(ClientError::ConnectionFailure)?;
        let opening_correlation = 1_u64;
        let open = v1::ApplicationSessionRequest {
            correlation_id: opening_correlation,
            request: Some(v1::application_session_request::Request::Open(
                identity.framed_open_request(
                    generate_request_id()
                        .map_err(ClientError::IdentifierGeneration)?
                        .into_bytes()
                        .to_vec(),
