//! `@generated` fixture-scoped WP-130 bindings for the canonical POC contract.
//!
//! Reproduce this file with `./scripts/generate-rust-sdk`. The wrapper first
//! runs the typed compiler fixture check that owns the fixture-to-bundle proof.
//! The fixture-scoped renderer consumes and validates the identities and shapes
//! in `bundle-metadata.txt`, `command-plans.txt`, and `lineage-ledger.txt`; it
//! rejects unsupported shape drift instead of inventing client semantics, but
//! does not decode `bundle.bin`.
//!
