...
impl RiffDbClient {
    ...
    /// Starts or resumes one immutable exact installation campaign with bounded retry.
    pub async fn start_application_installation_with_retry(
        &mut self,
        start: &StartApplicationInstallation,
        attempt_budget: AttemptBudget,
        metadata: &CallMetadata,
    ) -> Result<v1::StartApplicationInstallationResponse, ClientError> {
        let mut request_ids = SystemIdSource::new();
        let mut retry = RetryState::new(attempt_budget);
        loop {
            if !retry.begin_submission() {
