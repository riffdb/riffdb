...
//! Hostile and fragmented private diagnostic-frame semantics.

use riffdb_api_exclusive::{
    DIAGNOSTIC_CREDENTIAL_BYTES, DIAGNOSTIC_MAGIC, DiagnosticServerStages, encode_response,
    read_open, read_opened, read_request, read_response, write_open, write_opened, write_request,
};
...
async fn checked_open_and_frames_survive_fragmented_duplex_carriage() {
    ...
    let client_task = tokio::spawn(async move {
        write_open(&mut client, &credential, b"alpha", b"exact-session")
            .await
            .unwrap();
        assert_eq!(read_opened(&mut client).await.unwrap(), b"opened");
    ...
    write_opened(&mut server, b"opened").await.unwrap();
