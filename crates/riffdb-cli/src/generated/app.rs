...
use riffdb_client_rust::{
    ...
    ProjectedResponseEncoding, ProjectedSortDirection, RestoreArchivedBackup, RestoreOfflineBackup, RetireOfflineBackup,
...
use crate::cli::{
    FollowerCommand, FollowerSelectionArgs, AgentCommand, ApplicationCommand, ApplicationLanguage, BackupCommand, CapabilityCommand, Cli,
...
use crate::output::{
    ...
    render_projection, render_archive_restore_maintenance_start, render_restore_maintenance_start, render_retire_maintenance_start,
...
async fn dispatch(
    ...
    match command {
        ...
        TopLevel::Follower { command } => follower_command(command, config, environment).await,
        ...
        TopLevel::Storage { command: StorageCommand::Restore { name, archive, stop_at_sequence, confirm_replace_current_database } } => {
            archive_restore_command(name, archive, stop_at_sequence, confirm_replace_current_database, config, environment).await
        }
...
fn query_result_value_json(value: &v1::Value) -> Option<serde_json::Value> {
    use v1::value::Kind;

    match value.kind.as_ref()? {
        Kind::ListValue(list) => Some(serde_json::json!({
            "type": "list",
            "values": list
                .values
                .iter()
                .map(query_result_value_json)
                .collect::<Option<Vec<_>>>()?,
        })),
        Kind::RecordValue(record) if record.fields.iter().all(|field| field.field_id.is_none()) => {
            ...
            let fields = record
                ...
                .map(|field| {
                    ...
                    Some(serde_json::json!({
                        "name": field.name,
                        "value": query_result_value_json(field.value.as_ref()?)?,
                    }))
                })
                .collect::<Option<Vec<_>>>()?;
            Some(serde_json::json!({"type": "record", "fields": fields}))
        }
        _ => serde_json::to_value(crate::value::OutputValue(value)).ok(),
    }
}
...
const fn command_identity(command: &TopLevel) -> CommandIdentity {
    match command {
        ...
        TopLevel::Storage { command: StorageCommand::Restore { .. } } => CommandIdentity::StorageRestore,
        TopLevel::Storage {
...
async fn archive_restore_command(
    name: String, archive: String, stop_at_sequence: Option<u64>, confirm_replace: bool,
    config: &EffectiveConfig, environment: &dyn Environment,
) -> Terminal {
    let identity = CommandIdentity::StorageRestore;
    let name = match BackupNameV1::new(name) { Ok(name) => name, Err(_) => return invalid_input(identity) };
    let archive = match riffdb_types::ArchiveNameV1::new(archive) { Ok(name) => name, Err(_) => return invalid_input(identity) };
    let stop = match stop_at_sequence {
        None => riffdb_types::ArchiveRestoreStopV1::LastArchived,
        Some(sequence) => match riffdb_types::CommitSequence::new(sequence) {
            Some(sequence) => riffdb_types::ArchiveRestoreStopV1::AtApplicationSequence(sequence),
            None => return invalid_input(identity),
        },
    };
    let operation_id = match generate_offline_maintenance_operation_id() {
        Ok(id) => id, Err(error) => return client_error(identity, &ClientError::IdentifierGeneration(error)),
    };
    ...
    let restore = RestoreArchivedBackup::new(operation_id, name, archive, stop, restore_confirmation(confirm_replace));
    let attempts = AttemptBudget::new(config.max_attempts).expect("configuration bound");
    match client.restore_archived_backup_with_retry(&restore, attempts, &metadata).await {
        Ok(response) => render_archive_restore_maintenance_start(&response),
        Err(ClientError::OutcomeUnknown(_)) => maintenance_uncertain(identity, operation_id),
        Err(error) => client_error(identity, &error),
    }
}
...
fn storage_command(command: StorageCommand) -> Terminal {
    match command {
        // Network maintenance is dispatched by the asynchronous command router.
        StorageCommand::Restore { .. } => invalid_input(CommandIdentity::StorageRestore),
...
fn follower_selection(target: FollowerSelectionArgs) -> Result<v1::ReplicationFollowerTarget, ()> {
    let database_id = parse_uuid_v7(&target.database_id).ok_or(())?;
    if target.hold_id.len() != 32 || !target.hold_id.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte)) { return Err(()); }
    let hold_id = parse_opaque_hex(&target.hold_id, 16)?;
    if hold_id.len() != 16 || hold_id.iter().all(|byte| *byte == 0) { return Err(()); }
    Ok(v1::ReplicationFollowerTarget { database_id: database_id.to_vec(), history_incarnation: target.history_incarnation.get(), leadership_epoch: target.leadership_epoch.get(), hold_id })
}
...
async fn follower_command(command: FollowerCommand, config: &EffectiveConfig, environment: &dyn Environment) -> Terminal {
    ...
    let (register, retire) = match command {
        ...
        FollowerCommand::Retire { target, registration_generation } => {
            let target = match follower_selection(target) { Ok(target) => target, Err(()) => return invalid_input(identity) };
            (None, Some(v1::RetireFollowerRequest { request_id, target: Some(target), registration_generation: registration_generation.get() }))
        }
    };
    let mut client = match connect(config).await { Ok(client) => client, Err(error) => return client_error(identity, &error) };
    match (register, retire) {
        (Some(request), None) => match client.register_follower(request, &metadata).await {
            Ok(response) => match response.result {
                Some(v1::register_follower_response::Result::Receipt(receipt)) => render_follower_receipt(identity, &receipt),
                Some(v1::register_follower_response::Result::Refusal(refusal)) => render_follower_refusal(identity, refusal),
                None => local_error(identity, "output_render_failed", "output rendering failed"),
            },
            Err(error) => client_error(identity, &error),
        },
        (None, Some(request)) => match client.retire_follower(request, &metadata).await {
            Ok(response) => match response.result {
                Some(v1::retire_follower_response::Result::Receipt(receipt)) => render_follower_receipt(identity, &receipt),
                Some(v1::retire_follower_response::Result::Refusal(refusal)) => render_follower_refusal(identity, refusal),
                None => local_error(identity, "output_render_failed", "output rendering failed"),
            },
            Err(error) => client_error(identity, &error),
        },
        _ => invalid_input(identity),
    }
}

fn render_follower_receipt(command: CommandIdentity, receipt: &v1::FollowerAdministrationReceipt) -> Terminal {
    #[derive(Serialize)]
    struct Summary { status: &'static str, administration_sequence: String, registration_generation: String }
    let status = if receipt.replayed { "replayed" } else { "applied" };
    success(command, status, &Summary { status, administration_sequence: receipt.administration_sequence.to_string(), registration_generation: receipt.registration_generation.to_string() })
}
fn render_follower_refusal(command: CommandIdentity, refusal: i32) -> Terminal {
    let (code, message) = match v1::FollowerAdministrationRefusal::try_from(refusal) {
        Ok(v1::FollowerAdministrationRefusal::LineageMismatch) => ("lineage_mismatch", "selected source lineage is no longer current"),
        Ok(v1::FollowerAdministrationRefusal::RegistrationConflict) => ("registration_conflict", "registration has another immutable policy or release action"),
        Ok(v1::FollowerAdministrationRefusal::RegistrationMissingOrStale) => ("registration_missing_or_stale", "the exact registration generation is unavailable"),
        Ok(v1::FollowerAdministrationRefusal::ExpiryReached) => ("expiry_reached", "the selected application-sequence expiry has been reached"),
        Ok(v1::FollowerAdministrationRefusal::CapacityExhausted) => ("capacity_exhausted", "the follower registration capacity is exhausted"),
...
mod tests {
    ...
    #[test]
    // req: REP-006
    fn follower_cli_preserves_full_width_receipts_and_redacted_closed_refusals() {
        for replayed in [false, true] {
            let value = v1::FollowerAdministrationReceipt { administration_sequence: u64::MAX, registration_generation: u64::MAX - 1, replayed };
            let mut stdout = Vec::new(); let mut stderr = Vec::new();
            assert_eq!(render_follower_receipt(CommandIdentity::FollowerRegister, &value).emit(OutputMode::Json, &mut stdout, &mut stderr), ExitCode::SUCCESS);
        ...
        for (value, code) in [(1, "lineage_mismatch"), (2, "registration_conflict"), (3, "registration_missing_or_stale"), (4, "expiry_reached"), (5, "capacity_exhausted"), (99, "output_render_failed")] {
            let mut stdout = Vec::new(); let mut stderr = Vec::new();
            assert_ne!(render_follower_refusal(CommandIdentity::FollowerRetire, value).emit(OutputMode::Json, &mut stdout, &mut stderr), ExitCode::SUCCESS);
            let output: serde_json::Value = serde_json::from_slice(&stdout).unwrap();
    ...
    #[test]
    // req: REP-006
    fn follower_cli_refuses_noncanonical_or_zero_hold_identity() {
        let selection = |hold: String| FollowerSelectionArgs {
            database_id: "018f22a1-7b3c-7def-8123-456789abcdef".into(),
            history_incarnation: std::num::NonZeroU64::MIN,
            leadership_epoch: std::num::NonZeroU64::MIN, hold_id: hold,
        };
        assert!(follower_selection(selection("71".repeat(16))).is_ok());
        for hold in ["00".repeat(16), "AA".repeat(16), "71".repeat(15), "71".repeat(17), "private-path".into()] {
            assert!(follower_selection(selection(hold)).is_err());
        }
    }
    ...
    // req: OQ-113
    #[test]
    fn named_query_json_preserves_bounded_nested_result_records() {
        let value = v1::Value {
                ...
                values: vec![v1::Value {
                    kind: Some(v1::value::Kind::RecordValue(v1::ValueRecord {
                        fields: vec![v1::ValueField {
                            ...
                            value: Some(v1::Value {
                                kind: Some(v1::value::Kind::U64Value(7)),
                            }),
                        }],
                    })),
                }],
