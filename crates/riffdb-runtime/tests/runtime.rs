...
use riffdb_runtime::{
    ExecutionFault, ExecutionResult, TransactionContext, execute_command,
...
/// `execute_command_with_facts` accepts a proof the caller already holds, so
/// the bind that a local derivation used to give by construction must be
/// enforced: a proof derived from different input is rejected rather than
/// trusted, and the deriving entry point stays equivalent to it.
#[test]
fn supplied_input_facts_are_rejected_unless_bound_to_the_executed_command() {
    ...
    // Same plan, different rows: a valid proof that belongs to another command.
    let foreign_input = input_record(
        plan.input().record(),
        [
            ("request_id", CanonicalValue::Uuid([0x92; 16])),
            ("tenant_id", CanonicalValue::Uuid(tenant)),
            (
                "row_ids",
                CanonicalValue::List(
    ...
    let foreign_facts =
        derive_input_command_facts(plan, foreign_input.clone()).expect("foreign facts");
    ...
    // A proof bound to different input is refused, not used.
    assert!(matches!(
        execute_command_with_facts(
            &bundle,
            &input,
    ...
    // The bound proof produces exactly what deriving it locally produces.
    let supplied = execute_command_with_facts(
        &bundle,
        &input,
        &facts,
        &snapshot,
    ...
    .expect("bound proof evaluates");
    let derived = execute_command(&bundle, &input, &snapshot, &context, EvaluationBudget::v1())
        .expect("deriving entry evaluates");
    let (
        ExecutionResult::CommitRequired(supplied),
        ExecutionResult::CommitRequired(derived),
    ) = (supplied, derived)
    else {
        panic!("bounded delete requires commit");
    };
