// Promotion requires current administrative authority initially and again after drain.
// req: REP-005
use super::*;
...
#[test]
fn replication_promotion_requires_current_administrative_permission_and_exact_database() {
    ...
    for unrelated in [
        ...
        changed.grant = administrative_grant(
            vec![CapabilityPermissionV1::unparameterized(unrelated).unwrap()],
            Vec::new(),
        );
        assert_eq!(
            check(&changed, database_id(), 15),
...
#[test]
fn replication_promotion_refuses_scoped_and_application_administrators() {
    let permission =
...
#[test]
fn replication_promotion_preparation_binds_request_and_reloads_before_every_retry() {
    use riffdb_auth::{CurrentCapabilityResolutionError, ReplicationPromotionRequestV1};
    ...
    use riffdb_types::{
        ...
        ReplicationPromotionOperationId, ReplicationSourceHoldIdV1,
    };
    ...
    let operation_id =
        ReplicationPromotionOperationId::from_unix_milliseconds_and_random(4, [4; 10]).unwrap();
    ...
    let requests = [
        ...
        ReplicationPromotionRequestV1::new(
            ...
            target,
            ChangelogTransactionSequence::new(8).unwrap(),
        ),
    ];
    ...
    for request in requests {
        let ReplicationPromotionDecision::Allow(proof) = authorizer
            .authorize_replication_promotion(fixture.authenticated_principal(), request)
            .unwrap()
        else {
        ...
        assert_eq!(
            format!("{proof:?}"),
            "AuthorizedReplicationPromotionPreparation([REDACTED])"
        );
        let current = inner
    ...
    for request in requests {
        assert!(matches!(
            authorizer
                .authorize_replication_promotion(fixture.authenticated_principal(), request)
                .unwrap(),
            ReplicationPromotionDecision::Deny(PolicyCode::InactiveOrStaleCapability)
        ));
    }
