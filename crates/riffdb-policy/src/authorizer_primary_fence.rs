//! Fresh preparation for an explicit, audited primary fence.
use super::*;
...
pub struct AuthorizedPrimaryFencePreparation {
...
impl AuthorizedPrimaryFencePreparation {
    ...
    pub fn reauthorize(
        ...
        now: Timestamp,
    ) -> Result<AuthorizedPrimaryFence, PolicyCode> {
...
#[derive(Debug)]
pub struct AuthorizedPrimaryFence {
    preparation: AuthorizedPrimaryFencePreparation,
    timestamp: Timestamp,
}
impl AuthorizedPrimaryFence {
    /// Lowers the unchanged request/principal and the one final audit timestamp.
    #[must_use]
    pub fn into_parts(self) -> (AuthorizedPrimaryFencePreparation, Timestamp) {
        (self.preparation, self.timestamp)
    }
}

impl fmt::Debug for AuthorizedPrimaryFencePreparation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("AuthorizedPrimaryFencePreparation([REDACTED])")
    }
}

/// Closed preparation result; denial grants no fencing mutation.
#[derive(Debug)]
pub enum PrimaryFenceDecision {
    /// Exact request passed fresh initial authorization.
    Allow(Box<AuthorizedPrimaryFencePreparation>),
    ...
    T: AuthorizationTelemetry + ?Sized,
{
    ...
    /// fence request. Streaming and capability-administration permission do not grant fencing.
    pub fn authorize_primary_fence(
        &self,
        principal: &AuthenticatedPrincipal,
        request: PrimaryFenceRequestV1,
    ) -> Result<PrimaryFenceDecision, AuthorizationError> {
        ...
        match evaluate(
            ...
            Ok(()) => Ok(PrimaryFenceDecision::Allow(Box::new(
            ...
            Err(code) => {
                ...
                Ok(PrimaryFenceDecision::Deny(code))
            }
        }
    }
}
