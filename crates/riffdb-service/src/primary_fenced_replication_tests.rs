...
fn primary_fenced_control_denial_requires_terminal_audit() {
    run_async(async {
        for audit_outage in [false, true] {
            ...
            let target = riffdb_types::ReplicationFollowerAuditTargetV1::new(
                crate::primary_admission_test_support::database_id(),
                1,
            ...
            for record in h.audit_records(0xa3) {
                assert_eq!(record.operation(), ServiceOperationV1::RegisterFollower);
