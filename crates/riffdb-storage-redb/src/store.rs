...
/// Registry digest at the entity-reference branch base (post-audit-index current).
...
impl RedbStore {
    ...
    pub(crate) fn complete_partition_index_generation_migration(&self) -> Result<(), StorageError> {
        ...
        if observed == SchemaHash::from_bytes(PRE_INDEX_GENERATION_REGISTRY_DIGEST)
            || observed == SchemaHash::from_bytes(PRE_HISTORY_INCARNATION_REGISTRY_DIGEST)
            || observed == SchemaHash::from_bytes(PRE_AUDIT_REQUEST_INDEX_REGISTRY_DIGEST)
        {
            migrate_audit_request_index(&self.shared)?;
            publish_record_registry(
                &self.shared,
    ...
    loop {
        ...
        let mut reached_end = true;
        {
            ...
            for row in &mut rows {
                ...
                // Self-contained: hashes come from embedded post-images being
                // removed during decode; no ENTITIES join.
